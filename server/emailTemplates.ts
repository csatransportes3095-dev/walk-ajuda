/**
 * Templates de e-mail profissionais — Walk Ajuda
 * Todos os e-mails do sistema usam estas funções para garantir
  ...
  <tr><td align="center">
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
      ...
      <tr><td style="background:linear-gradient(135deg,#1a0a2e 0%,#0d0d1a 100%);border-radius:12px 12px 0 0;padding:28px 32px;border-bottom:2px solid #a855f7;text-align:center;">
        <div style="font-size:26px;font-weight:900;color:#a855f7;letter-spacing:2px;text-transform:uppercase;">WALK AJUDA</div>
        <div style="font-size:12px;color:#888;margin-top:4px;letter-spacing:1px;">walkajuda.com</div>
      </td></tr>
      ...
      <tr><td style="background:#080810;border-radius:0 0 12px 12px;padding:20px 32px;border:1px solid #1e1e3a;border-top:1px solid #a855f720;text-align:center;">
        <p style="color:#555;font-size:11px;margin:0 0 6px;">Este e-mail foi enviado automaticamente pelo sistema Walk Ajuda.</p>
        <p style="color:#555;font-size:11px;margin:0;">Dúvidas? Acesse <a href="https://walkajuda.com" style="color:#a855f7;text-decoration:none;">walkajuda.com</a></p>
      </td></tr>
...
export function emailStatusCliente(opts: {
  ...
  if (orderNumber) resumoRows.push(infoRow('Nº Pedido:', `<strong>#${orderNumber}</strong>`));
  if (service) resumoRows.push(infoRow('Serviço:', service));
  ...
  const descBlock = description ? `
    <div style="background:#0f1a10;border-left:3px solid #22c55e;border-radius:0 8px 8px 0;padding:16px 18px;margin-bottom:20px;">
      <p style="color:#22c55e;font-size:11px;font-weight:700;margin:0 0 10px;text-transform:uppercase;letter-spacing:1px;">📋 Instruções / Informações</p>
  ...
  const noteBlock = note ? `
    <div style="background:#1a1000;border-left:3px solid #f59e0b;border-radius:0 8px 8px 0;padding:16px 18px;margin-bottom:20px;">
      <p style="color:#f59e0b;font-size:11px;font-weight:700;margin:0 0 10px;text-transform:uppercase;letter-spacing:1px;">📝 Observação do Administrador</p>
  ...
  const content = `
    ...
    <p style="color:#888;font-size:13px;margin:0 0 12px;text-transform:uppercase;letter-spacing:1px;font-weight:600;">Atualização do seu pedido:</p>
    ...
    ${ctaButton('📱 Acompanhar Meu Pedido', 'https://walkajuda.com/acompanhar')}
    <p style="color:#555;font-size:11px;text-align:center;margin:4px 0 0;">Use seu telefone para consultar em walkajuda.com/acompanhar</p>
    ${pixel}
  `;
...
export function emailNovoPedidoAdmin(opts: {
  ...
  const { clientName, phone, service, option, customerNumber, orderNumber, email, cpf, extra, answers, documents, paymentProofUrl, city, referrer, carDocumentYear } = opts;
  ...
  if (option) rows.push(infoRow('Opção:', option));
  if (customerNumber) rows.push(infoRow('Cadastro:', `*${customerNumber}`));
  if (orderNumber) rows.push(infoRow('Nº Pedido:', `<strong>#${orderNumber}</strong>`));
  ...
  if (city) infoItems.push(`🏙️ <strong>Cidade:</strong> ${city}`);
  if (referrer) infoItems.push(`👤 <strong>Indicado por:</strong> ${referrer}`);
  if (carDocumentYear) infoItems.push(`📅 <strong>Ano desejado:</strong> ${carDocumentYear}`);
  ...
  const infoBlock = infoItems.length > 0 ? `
    <div style="background:#0a1020;border-left:3px solid #3b82f6;border-radius:0 8px 8px 0;padding:14px 16px;margin-top:16px;">
      <p style="color:#3b82f6;font-size:11px;font-weight:700;margin:0 0 10px;text-transform:uppercase;letter-spacing:1px;">ℹ️ Informações do Cliente</p>
  ...
  const pixBlock = paymentProofUrl ? `
    <div style="background:#0f1a10;border-left:3px solid #22c55e;border-radius:0 8px 8px 0;padding:14px 16px;margin-top:16px;">
      ...
      <p style="color:#ccc;font-size:13px;margin:0;"><a href="${paymentProofUrl}" style="color:#22c55e;text-decoration:underline;">📎 Ver comprovante</a></p>
    </div>` : `
    <div style="background:#1a1000;border-left:3px solid #f59e0b;border-radius:0 8px 8px 0;padding:14px 16px;margin-top:16px;">
      <p style="color:#f59e0b;font-size:11px;font-weight:700;margin:0 0 8px;text-transform:uppercase;letter-spacing:1px;">💰 Comprovante PIX</p>
  ...
  const content = `
    <div style="background:#0f1a10;border:1px solid #22c55e40;border-radius:8px;padding:10px 16px;margin-bottom:20px;text-align:center;">
      <span style="color:#22c55e;font-size:13px;font-weight:700;">🆕 NOVO PEDIDO RECEBIDO</span>
    </div>
    ...
    ${ctaButton('Ver no Painel', 'https://walkajuda.com/admin')}
  `;
...
export function emailPedidoRecebidoCliente(opts: {
  ...
  const { siteTitle, customerName, service, orderNumber, pin } = opts;
  ...
  const rows = [
    infoRow('Serviço:', service),
    ...(orderNumber ? [infoRow('Nº Pedido:', `<strong>#${orderNumber}</strong>`)] : []),
  ];
...
export function emailCadastroFinalizadoAdmin(opts: {
  ...
  const { name, phone, service, email, cpf, extra } = opts;

  const rows = [
    infoRow('Nome:', `<strong>${name}</strong>`),
    infoRow('Telefone:', phone),
    ...(service ? [infoRow('Serviço:', service)] : []),
  ...
  const content = `
    <div style="background:#0a1020;border:1px solid #3b82f640;border-radius:8px;padding:10px 16px;margin-bottom:20px;text-align:center;">
      <span style="color:#3b82f6;font-size:13px;font-weight:700;">📋 CADASTRO FINALIZADO</span>
    </div>
...
/** E-mail de INÍCIO DE CADASTRO para o admin */
export function emailInicioCadastroAdmin(opts: {
  phone: string;
  service?: string;
  extra?: string;
}): string {
  const { phone, service, extra } = opts;
  ...
  const content = `
    <div style="background:#1a1000;border:1px solid #f59e0b40;border-radius:8px;padding:10px 16px;margin-bottom:20px;text-align:center;">
      <span style="color:#f59e0b;font-size:13px;font-weight:700;">🆕 NOVO CLIENTE INICIOU CADASTRO</span>
    </div>
...
/** E-mail de INDICAÇÃO para o indicador */
export function emailIndicacaoSucesso(opts: {
  referrerName?: string;
  referredName: string;
  service?: string;
}): string {
  const { referrerName, referredName, service } = opts;

  const greeting = referrerName
    ? `Olá, <strong style="color:#e0e0e0;">${referrerName}</strong>!`
    : 'Olá!';
  ...
  const content = `
    ...
    <div style="background:#0f1a10;border:1px solid #22c55e40;border-radius:8px;padding:14px 16px;margin-bottom:20px;text-align:center;">
      <span style="color:#22c55e;font-size:14px;font-weight:700;">🎉 Sua indicação deu certo!</span>
    </div>

    <p style="color:#ccc;font-size:14px;line-height:1.7;margin:0 0 16px;">
      <strong style="color:#e0e0e0;">${referredName}</strong> fez um pedido${service ? ` de <strong>${service}</strong>` : ''} usando sua indicação.
    </p>

    <p style="color:#888;font-size:13px;line-height:1.7;margin:0;">Obrigado por indicar a Walk Ajuda! Continue indicando e ajudando mais pessoas.</p>

    ${ctaButton('Acessar Walk Ajuda', 'https://walkajuda.com')}
  `;
...
export function emailComprovantePix(opts: {
  ...
  const { clientName, phone, service, extra } = opts;
  ...
  const extraBlock = extra ? `
    <div style="background:#0a1020;border-left:3px solid #3b82f6;border-radius:0 8px 8px 0;padding:14px 16px;margin-top:16px;">
      <p style="color:#3b82f6;font-size:11px;font-weight:700;margin:0 0 8px;text-transform:uppercase;letter-spacing:1px;">Informações do Comprovante</p>
      <p style="color:#ccc;font-size:13px;margin:0;line-height:1.8;">${nl2br(extra)}</p>
    </div>` : '';

  const content = `
    <div style="background:#0a1020;border:1px solid #3b82f640;border-radius:8px;padding:10px 16px;margin-bottom:20px;text-align:center;">
      <span style="color:#3b82f6;font-size:13px;font-weight:700;">💳 COMPROVANTE PIX RECEBIDO</span>
    </div>
...
export function emailStatusAdmin(opts: {
  ...
  if (service) rows.push(infoRow('Serviço:', service));
  if (option) rows.push(infoRow('Opção:', option));
  if (previousStatus) rows.push(infoRow('Status Anterior:', `<span style="color:#888;">${previousStatus}</span>`));

  const noteBlock = note ? `
    <div style="background:#1a1000;border-left:3px solid #f59e0b;border-radius:0 8px 8px 0;padding:14px 16px;margin-top:16px;">
      <p style="color:#f59e0b;font-size:11px;font-weight:700;margin:0 0 8px;text-transform:uppercase;letter-spacing:1px;">📝 Observação</p>
