...
export const appRouter = router({
  ...
  // === PRODUTOS / CARDS DE SERVIÁƒâ€¡O ===
  products: router({
    // Público: listar produtos ativos com opçÁµes, perguntas, documentos e tiers de garantia por opção
    ...
    // Admin: listar todos os produtos com opçÁµes, perguntas, documentos e tiers de garantia por opção
    list: adminProcedure.query(async () => {
  ...
  productOptions: router({
    ...
    update: adminProcedure
      ...
      .input(z.object({ id: z.number() }))
      .mutation(async ({ input }) => {
        // Deletar documentos associados Á  opção
  ...
  // === TIERS DE GARANTIA POR OPÁƒâ€¡ÁƒÆ’O ===
  ...
  // === DOCUMENTOS DINÁƒâ€šMICOS POR OPÁƒâ€¡ÁƒÆ’O ===
  ...
  // === PERGUNTAS POR OPÁƒâ€¡ÁƒÆ’O DE COMPRA ===
          ...
          if (input.email) {
            try {
                  ...
                  if (ans.length > 0) {
                    answersClientHtml = `
                      <div style="margin:16px 0;">
                        <p style="color:#a855f7;font-weight:bold;font-size:14px;margin-bottom:8px;">Á°Å¸"â€¹ RESPOSTAS DO FORMULÁRIO</p>
                        ${ans.map(a => {
                          const d = a.depth || 0;
                          const ml = d * 16;
                          const prefix = d > 0 ? 'ââ€ Â³ ' : '';
              ...
              await sendEmailWithTimeout({
                from: '"H2 COLOMBIANO" <h2@h2colombiano.com>',
                to: input.email,
                subject: `âÅ“â€¦ Pedido Recebido — ${emailBranding.siteTitle}`,
  ...
  customers: router({
    ...
    register: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        // SEGURANÁƒâ€¡A: foto é obrigatória no servidor
          ...
          try {
            ...
            if (referrer?.email) {
              ...
              await transporter.sendMail({
                from: '"H2 COLOMBIANO" <h2@h2colombiano.com>',
                to: referrer.email,
                subject: `Á°Å¸Å½â€° Sua indicação deu certo! ${safeInput.name} fez um pedido`,
        ...
        void (async () => {
          try {
            ...
            await transporter.sendMail({
              from: '"H2 COLOMBIANO" <h2@h2colombiano.com>',
              to: emailTo,
              subject: `âÅ“â€¦ Cadastro finalizado — ${safeInput.name} (${safeInput.phone})`,
        ...
        void (async () => {
         try {
          ...
          await transporter.sendMail({
            from: '"H2 COLOMBIANO" <h2@h2colombiano.com>',
            to: emailTo,
            subject: `Á°Å¸"Â¸ Novo cliente iniciou cadastro — ${input.phone}`,
    ...
    setProductAccess: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!db) return { success: false };
        const phone = input.phone.replace(/\D/g, '');
        // Remover permissÁµes antigas e inserir as novas
    ...
    addBlockingNote: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const newNote = `[${timestamp}] âÅ¡Â Á¯Â¸Â Recusou: "${input.question}" ââ€ ' selecionou "${input.answer}"\n`;
  ...
  raffles: router({
    ...
    chooseNumber: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const raffleNotifTitle = `Á°Å¸Å½Â« Novo participante no sorteio!`;
        try {
  ...
  adminAuth: router({
    ...
    requestUnlock: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const ip = (ctx.req.headers['x-forwarded-for'] as string)?.split(',')[0]?.trim() || ctx.req.socket.remoteAddress || 'unknown';
        const title = 'Á°Å¸â€Â Solicitação de Desbloqueio Admin';
  ...
  orderStatus: router({
    ...
    listOrders: adminProcedure.query(async () => {
      ...
      // ---- Pedidos ÓRFÁƒÆ’OS ----
      // Alguns pedidos têm histórico (orderStatusHistory) cujo registrationId NÁƒÆ’O existe em
      // accessCodePhones (dados importados/migrados de versÁµes anteriores). Sem isto eles
      ...
      .mutation(async ({ input }) => {
        ...
        if (input.customerEmail && !isCustomerBlocked) {
          try {
            ...
            const emailBranding = await getEmailBranding();
            const noteHtml = input.note ? `<div style="background:#0d2b1a;border:1px solid #22c55e40;border-radius:8px;padding:16px;margin-bottom:20px;"><p style="color:#22c55e;font-size:12px;font-weight:bold;margin:0 0 8px;">Á°Å¸"â€¹ Observação:</p><p style="color:#ccc;font-size:14px;margin:0;white-space:pre-line;">${input.note}</p></div>` : '';
      ...
      .mutation(async ({ input }) => {
        ...
        if (input.customerEmail && !input.skipEmail && !isCustomerBlocked2) {
          try {
            ...
            if (input.orderNumber) pedidoRows.push(`<tr><td style="color:#888;font-size:12px;padding:4px 8px 4px 0;">NÁ‚Âº Pedido</td><td style="color:#fff;font-size:13px;font-weight:bold;padding:4px 0;">#${input.orderNumber}</td></tr>`);
            ...
            if (input.deliveryEstimate) {
              ...
              pedidoRows.push(`<tr><td style="color:#888;font-size:12px;padding:4px 8px 4px 0;">Previsão</td><td style="color:#22c55e;font-size:13px;font-weight:bold;padding:4px 0;">Á°Å¸"â€¦ ${previsaoU}</td></tr>`);
            }
    ...
    getMyStatus: publicProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        // Função de divisão em sub-pedidos — IDÁƒÅ NTICA Á  do admin (history em ASC)
    ...
    resendEmail: adminProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          ...
          if (input.orderNumber) reRows.push(`<tr><td style="color:#888;font-size:12px;padding:4px 8px 4px 0;">NÁ‚Âº Pedido</td><td style="color:#fff;font-size:13px;font-weight:bold;padding:4px 0;">#${input.orderNumber}</td></tr>`);
          ...
          if (input.deliveryEstimate) {
            ...
            reRows.push(`<tr><td style="color:#888;font-size:12px;padding:4px 8px 4px 0;">Previsão</td><td style="color:#22c55e;font-size:13px;font-weight:bold;padding:4px 0;">Á°Å¸"â€¦ ${previsaoR}</td></tr>`);
          }
            ...
            if (row?.referrerPhone) {
              ...
              if (referrer?.email) {
                ...
                await transporter.sendMail({
                  ...
                  to: referrer.email,
                  subject: `âÅ“â€¦ Sua comissão${commText} foi paga! - ${siteTitle}`,
                  html: `
                    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#0f172a;color:#e2e8f0;border-radius:12px;overflow:hidden">
                      <div style="background:linear-gradient(135deg,#7c3aed,#4f46e5);padding:28px 24px;text-align:center">
                        <h1 style="margin:0;font-size:22px;color:#fff">âÅ“â€¦ Comissão Paga!</h1>
                        <p style="margin:8px 0 0;color:#c4b5fd;font-size:14px">${siteTitle}</p>
                      </div>
                      <div style="padding:24px">
                        <p style="font-size:15px;margin:0 0 16px">Olá <strong>${row.referrerName || referrer.name || 'indicador'}</strong>! Á°Å¸Å½â€°</p>
                        ...
                        <p style="font-size:13px;color:#64748b;margin:16px 0 0">Obrigado por indicar! Continue indicando e ganhe mais. Á°Å¸'Âª</p>
                      </div>
                    </div>
                  `,
    ...
    // Admin: relatório de comissÁµes (pedidos com indicador)
      ...
      .mutation(async ({ input }) => {
        try {
          ...
          const commissionHtml = input.commissionValue && input.commissionValue > 0
            ? `<p style="margin:12px 0 8px;font-size:13px;color:#fcd34d;font-weight:bold;">Á°Å¸'Â° Comissão: R$ ${(input.commissionValue / 100).toFixed(2).replace('.', ',')}</p>`
            : '';
          await transporter.sendMail({
            from: '"H2 COLOMBIANO" <h2@h2colombiano.com>',
            to: referrer.email,
            subject: `[Reenvio] Á°Å¸Å½â€° Sua indicação deu certo! ${input.referredName} fez um pedido`,
            html: emailIndicacaoSucesso({
              ...emailBranding,
    ...
    createManualOrder: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!customer) {
          ...
          // Atualizar dados existentes — NÁƒÆ’O sobrescrever referredBy se cliente já existe
    ...
    createManualOrderMultiple: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!customer) {
          ...
          // NÁƒÆ’O sobrescrever referredBy se cliente já existe
          // Indicação só conta para clientes novos (primeiro cadastro)
          await updateCustomer(customer.id, {
            name: input.name,
            email: input.email,
        ...
        // 4. Criar UMA ÁƒÅ¡NICA entrada no histórico com todos os produtos concatenados e 1 número de pedido
        let orderNum: number | undefined;
        try { orderNum = await generateOrderNumber(); } catch (e) { console.error('[OrderNumber] Erro:', e); }
      ...
      .query(async ({ input }) => {
        ...
        return rows.map((r: any) => {
          ...
          if (r.folderName) {
            ...
            folderLabel = 'Á°Å¸"Â Arquivo';
            folderType = 'archived';
          } else if (Number(r.rgCnhApproved) === 1) {
            folderLabel = 'Á°Å¸ÂªÂ· RG/CNH Aprovado';
            folderType = 'rgcnh';
          } else {
            folderLabel = 'Á°Å¸"â€¹ Pedidos Ativos';
            folderType = 'active';
          }
          return {
            registrationId: Number(r.registrationId),
  ...
  // ââ€â‚¬ââ€â‚¬ AnotaçÁµes Internas do Admin por Pedido ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬
  orderNotes: router({
  ...
  // ââ€â‚¬ââ€â‚¬ Dados de Login Liberado por Pedido ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬
  loginData: router({
  ...
  // === SOLICITAÁƒâ€¡Áƒâ€¢ES DE DOCUMENTOS PENDENTES ===
  docRequests: router({
    // Admin: listar solicitaçÁµes de um pedido
    ...
    // Público: cliente busca solicitaçÁµes pendentes pelo telefone
    ...
    answer: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        // Notificar admin por e-mail
        const docNotifTitle = 'Á°Å¸"â€ž Documento respondido pelo cliente';
        try {
  ...
  raffleAccess: router({
    ...
    // Admin: obter configuraçÁµes do sorteio
    ...
    // Admin: salvar configuraçÁµes do sorteio
  ...
  security: router({
    reportPrintAttempt: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Na 3Á‚Âª tentativa: bloquear IP e número
        if (attempts >= 3) {
          if (clientIp && clientIp !== 'unknown') {
  ...
  // ââ€â‚¬ââ€â‚¬ââ€â‚¬ Controle Financeiro ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬
  financial: router({
  ...
  // ââ€â‚¬ââ€â‚¬ââ€â‚¬ Links de Indicação por Cliente ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬
  referral: router({
  ...
  // === FORMULÁRIO DINÁƒâ€šMICO - TELA DE ACOMPANHAMENTO ===
    ...
    create: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const [result] = await dbInst.insert(customFoldersTable).values({
          name: input.name,
          icon: input.icon ?? 'Á°Å¸"Â',
          color: input.color ?? '#8b5cf6',
          sortOrder: 0,
        });
        return { id: (result as any).insertId, name: input.name, icon: input.icon ?? 'Á°Å¸"Â', color: input.color ?? '#8b5cf6' };
      }),
  ...
  // ===================== CONFIGURAÁƒâ€¡ÁƒÆ’O DAS PASTAS FIXAS =====================
  folderConfig: router({
    // Buscar configuraçÁµes de todas as pastas fixas
    getAll: adminProcedure.query(async () => {
      ...
      for (const row of rows) {
        result[row.folderKey] = { id: row.id, name: row.name, icon: row.icon ?? 'Á°Å¸"Â', color: row.color ?? '#8b5cf6', tabOrder: row.tabOrder, hidden: row.hidden ?? 0 };
      }
      return result;
    }),
    ...
    save: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        await dbInst.update(folderConfigTable)
          .set({ name: input.name, icon: input.icon ?? 'Á°Å¸"Â', color: input.color ?? '#8b5cf6' })
  ...
  // ââ€â‚¬ââ€â‚¬ââ€â‚¬ ETAPAS INTERNAS ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬
  stages: router({
    list: adminProcedure.query(async () => {
  ...
  // BotÁµes extras da tela inicial do cliente (antes do login) — gerenciáveis pelo admin
  homeButtons: router({
    // Público: lista apenas os botÁµes ativos, em ordem (usado no WelcomeScreen)
  ...
  zohoConfig: router({
    ...
    getAuthUrl: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        console.log('[getAuthUrl] Sessão recuperada imediatamente:', retrieved ? 'SIM' : 'NÁƒÆ’O');
        if (retrieved) {
