...
export const appRouter = router({
  ...
  productOptions: router({
    ...
    update: adminProcedure
      ...
      .input(z.object({ id: z.number() }))
      .mutation(async ({ input }) => {
        // Deletar documentos associados à opção
  ...
  // === DOCUMENTOS DINÂMICOS POR OPÇÃO ===
        ...
        try {
          ...
          if (input.cpToken) {
            ...
            try {
              ...
              cpTokenValid = true; // Sessão válida — pular checkAccessCodeCanSubmit
            } catch {
          ...
          try {
            ...
            let whatsappMsg = `NOVO PEDIDO - WALK AJUDA`;
          ...
          try {
            const db2 = await (await import('./db')).getDb();
            if (db2 && input.phone) {
              // Normalizar telefone para apenas dígitos (ex: "(11) 99342-5253" -> "11993425253")
              ...
              if (regId) {
                ...
                // Os docs foram salvos com docRegId (último acesso do telefone), mas devem usar o regId do novo pedido
                  ...
                  if (ans.length > 0) {
                    answersClientHtml = `
                      <div style="margin:16px 0;">
                        <p style="color:#a855f7;font-weight:bold;font-size:14px;margin-bottom:8px;">📋 RESPOSTAS DO FORMULÁRIO</p>
                        ${ans.map(a => {
                          const d = a.depth || 0;
                          const ml = d * 16;
                          const prefix = d > 0 ? '↳ ' : '';
  ...
  customers: router({
    ...
    addBlockingNote: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const newNote = `[${timestamp}] ⚠️ Recusou: "${input.question}" → selecionou "${input.answer}"\n`;
  ...
  adminAuth: router({
    ...
    requestUnlock: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const ip = (ctx.req.headers['x-forwarded-for'] as string)?.split(',')[0]?.trim() || ctx.req.socket.remoteAddress || 'unknown';
        const title = '🔐 Solicitação de Desbloqueio Admin';
  ...
  orderStatus: router({
    ...
    listOrders: adminProcedure.query(async () => {
      ...
      // ---- Pedidos ÓRFÃOS ----
      // Alguns pedidos têm histórico (orderStatusHistory) cujo registrationId NÃO existe em
      ...
      .mutation(async ({ input }) => {
        ...
        if (input.customerEmail && !input.skipEmail && !isCustomerBlocked2) {
          try {
            ...
            const siteBaseUrl = 'https://walkajuda.com';
            const trackingPixelUrl = `${siteBaseUrl}/api/email-open/${trackingId}`;
            ...
            if (input.orderNumber) pedidoRows.push(`<tr><td style="color:#888;font-size:12px;padding:4px 8px 4px 0;">Nº Pedido</td><td style="color:#fff;font-size:13px;font-weight:bold;padding:4px 0;">#${input.orderNumber}</td></tr>`);
            ...
            if (input.deliveryEstimate) {
              ...
              pedidoRows.push(`<tr><td style="color:#888;font-size:12px;padding:4px 8px 4px 0;">Previsão</td><td style="color:#22c55e;font-size:13px;font-weight:bold;padding:4px 0;">📅 ${previsaoU}</td></tr>`);
            }
    ...
    getMyStatus: publicProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        // Função de divisão em sub-pedidos — IDÊNTICA à do admin (history em ASC)
    ...
    resendEmail: adminProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          // smtpPass não é mais necessário - usando Resend API via HTTPS
          ...
          if (input.orderNumber) reRows.push(`<tr><td style="color:#888;font-size:12px;padding:4px 8px 4px 0;">Nº Pedido</td><td style="color:#fff;font-size:13px;font-weight:bold;padding:4px 0;">#${input.orderNumber}</td></tr>`);
          ...
          if (input.deliveryEstimate) {
            ...
            reRows.push(`<tr><td style="color:#888;font-size:12px;padding:4px 8px 4px 0;">Previsão</td><td style="color:#22c55e;font-size:13px;font-weight:bold;padding:4px 0;">📅 ${previsaoR}</td></tr>`);
          }
    ...
    createManualOrder: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!customer) {
          ...
          // Atualizar dados existentes — NÃO sobrescrever referredBy se cliente já existe
    ...
    createManualOrderMultiple: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!customer) {
          ...
          // NÃO sobrescrever referredBy se cliente já existe
          // Indicação só conta para clientes novos (primeiro cadastro)
          await updateCustomer(customer.id, {
            name: input.name,
            email: input.email,
        ...
        // 4. Criar UMA ÚNICA entrada no histórico com todos os produtos concatenados e 1 número de pedido
        let orderNum: number | undefined;
        try { orderNum = await generateOrderNumber(); } catch (e) { console.error('[OrderNumber] Erro:', e); }
  ...
  // ── Anotações Internas do Admin por Pedido ─────────────────────────────────
  orderNotes: router({
  ...
  // ── Dados de Login Liberado por Pedido ─────────────────────────────────────
  loginData: router({
  ...
  security: router({
    reportPrintAttempt: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Na 3ª tentativa: bloquear IP e número
        if (attempts >= 3) {
          if (clientIp && clientIp !== 'unknown') {
  ...
  folders: router({
    ...
    create: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        return { id: (result as any).insertId, name: input.name, icon: input.icon ?? '📁', color: input.color ?? '#8b5cf6' };
      }),
  ...
  folderConfig: router({
    ...
    getAll: adminProcedure.query(async () => {
      ...
      for (const row of rows) {
        result[row.folderKey] = { id: row.id, name: row.name, icon: row.icon ?? '📁', color: row.color ?? '#8b5cf6', tabOrder: row.tabOrder, hidden: row.hidden ?? 0 };
      }
      return result;
    }),
    ...
    save: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        await dbInst.update(folderConfigTable)
          .set({ name: input.name, icon: input.icon ?? '📁', color: input.color ?? '#8b5cf6' })
