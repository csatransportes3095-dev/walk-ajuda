...
async function startServer() {
  ...
  app.use(async (req, res, next) => {
    // NÃ£o bloquear rotas admin (frontend) nem qualquer chamada tRPC de admin autenticado
    if (req.path.startsWith("/admin") || req.path.startsWith("/api/trpc/admin")) {
      return next();
    }
    // NÃ£o bloquear chamadas tRPC se o cookie admin_token estiver presente e vÃ¡lido
    ...
    if (ip && ip !== "unknown") {
      ...
      } catch (e) { /* silenciar erros de verificaÃ§Ã£o */ }
    }
    next();
  });
  ...
  app.get("/foto/:slug", async (req, res) => {
    ...
    try {
      ...
      if (!db) { res.status(503).send("Banco indisponÃ­vel"); return; }
      const rows = await db.select().from(adminMediaFiles).where(eqOp(adminMediaFiles.videoSlug, slug)).limit(1);
      if (!rows.length) { res.status(404).send("<h2>Imagem nÃ£o encontrada</h2>"); return; }
      const media = rows[0];
      const imageUrl = (media as any).url || '';
      if (!imageUrl) { res.status(502).send("URL da imagem nÃ£o encontrada"); return; }
  ...
  app.get("/video/:slug", async (req, res) => {
    ...
    try {
      ...
      if (!db) { res.status(503).send("Banco indisponÃ­vel"); return; }
      const rows = await db.select().from(adminMediaFiles).where(eqOp(adminMediaFiles.videoSlug, slug)).limit(1);
      if (!rows.length) { res.status(404).send("<h2>VÃ­deo nÃ£o encontrado</h2>"); return; }
      const media = rows[0];
      // Usar a URL direta do CloudFront salva no banco (sem assinatura, acesso pÃºblico)
      const videoUrl = (media as any).url || '';
      if (!videoUrl) { res.status(502).send("URL do vÃ­deo nÃ£o encontrada"); return; }
      const title = media.name.replace(/\.(mp4|webm|mov|avi)$/i, "");
      const canonicalUrl = `https://h2colombiano.com/video/${slug}`;
      res.set('Content-Type', 'text/html; charset=utf-8');
      res.send(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title} - H2 COLOMBIANO</title><meta property="og:title" content="${title}"><meta property="og:description" content="H2 COLOMBIANO - Atendimento rÃ¡pido para motoristas de app"><meta property="og:type" content="video.other"><meta property="og:url" content="${canonicalUrl}"><meta property="og:site_name" content="H2 COLOMBIANO"><meta property="og:video" content="${videoUrl}"><meta property="og:video:secure_url" content="${videoUrl}"><meta property="og:video:type" content="${media.mimeType || 'video/mp4'}"><meta property="og:video:width" content="1280"><meta property="og:video:height" content="720"><meta property="og:image" content="${videoUrl}"><meta name="twitter:card" content="player"><meta name="twitter:title" content="${title}"><meta name="twitter:description" content="H2 COLOMBIANO - Atendimento rÃ¡pido para motoristas de app"><meta name="twitter:player" content="${canonicalUrl}"><meta name="twitter:player:width" content="1280"><meta name="twitter:player:height" content="720"><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#000;display:flex;align-items:center;justify-content:center;min-height:100vh}video{width:100%;max-width:900px;max-height:100vh}</style></head><body><video controls autoplay playsinline preload="auto"><source src="${videoUrl}" type="${media.mimeType || 'video/mp4'}">Seu browser nÃ£o suporta vÃ­deo HTML5.</video></body></html>`);
    } catch (err) {
  ...
  // Rota de vÃ­deos pÃºblicos â€” pÃ¡gina HTML com player nativo
  app.get("/video/tutorial", async (_req, res) => {
    try {
      ...
      if (!forgeResp.ok) { res.status(502).send("Erro ao obter vÃ­deo"); return; }
      const { url } = await forgeResp.json() as { url: string };
      if (!url) { res.status(502).send("URL invÃ¡lida"); return; }
      res.set('Content-Type', 'text/html; charset=utf-8');
      res.send(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Tutorial H2 COLOMBIANO</title><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#000;display:flex;align-items:center;justify-content:center;min-height:100vh}video{width:100%;max-width:900px;max-height:100vh}</style></head><body><video controls autoplay playsinline preload="auto"><source src="${url}" type="video/mp4">Seu browser nÃ£o suporta vÃ­deo HTML5.</video></body></html>`);
    } catch (err) {
  ...
  // === ZOHO OAUTH CALLBACK - troca cÃ³digo por refresh token automaticamente ===
  app.get("/api/zoho-oauth-callback", async (req, res) => {
    const { code, state, error } = req.query as Record<string, string>;

    const errorHtml = (msg: string) => `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Erro</title></head><body style="font-family:sans-serif;background:#0f172a;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0"><div style="text-align:center;padding:40px;background:#1e293b;border-radius:12px;max-width:500px"><h2 style="color:#ef4444">âŒ Erro na autorizaÃ§Ã£o</h2><p style="color:#94a3b8;margin:16px 0">${msg}</p><p style="color:#64748b;font-size:13px">Feche esta aba e tente novamente.</p></div></body></html>`;
    const successHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Sucesso</title></head><body style="font-family:sans-serif;background:#0f172a;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0"><div style="text-align:center;padding:40px;background:#1e293b;border-radius:12px;max-width:500px"><h2 style="color:#22c55e">âœ… Token gerado com sucesso!</h2><p style="color:#94a3b8;margin:16px 0">ConfiguraÃ§Ã£o Zoho salva. Volte para o painel e clique em <strong>Atualizar</strong>.</p><p style="color:#64748b;font-size:13px">Pode fechar esta aba.</p><script>setTimeout(()=>window.close(),3000)</script></div></body></html>`;

    if (error) { res.send(errorHtml(`Zoho retornou: ${error}`)); return; }
    if (!code || !state) { res.send(errorHtml('ParÃ¢metros invÃ¡lidos na resposta do Zoho.')); return; }
    ...
    try {
      ...
      try {
        ...
        console.log('[ZohoOAuth] getPendingZohoOAuth resultado:', pending ? 'encontrado' : 'nÃ£o encontrado');
      } catch (e) {
        console.error('[ZohoOAuth] Erro ao recuperar sessÃ£o:', e);
        res.send(errorHtml(`Erro ao recuperar sessÃ£o: ${String(e)}`));
        return;
      }
      
      if (!pending) { 
        res.send(errorHtml('SessÃ£o expirada (mais de 10 min). Tente novamente.')); 
        return; 
      }

      console.log('[ZohoOAuth] Trocando cÃ³digo por refresh token');
      // Trocar cÃ³digo por refresh token
      ...
      if (!tokenData.refresh_token) { 
        res.send(errorHtml(`Zoho nÃ£o retornou refresh_token: ${tokenData.error || 'erro desconhecido'}`)); 
        return; 
      }

      console.log('[ZohoOAuth] Salvando configuraÃ§Ã£o no banco');
      // Salvar configuraÃ§Ã£o completa no banco
      try {
        ...
        console.log('[ZohoOAuth] ConfiguraÃ§Ã£o salva com sucesso');
      } catch (e) {
        console.error('[ZohoOAuth] Erro ao salvar configuraÃ§Ã£o:', e);
        res.send(errorHtml(`Erro ao salvar configuraÃ§Ã£o: ${String(e)}`));
        return;
      }

      console.log('[ZohoOAuth] Deletando sessÃ£o temporÃ¡ria');
  ...
  // Endpoint de teste para debugar sessÃ£o OAuth
  app.get("/api/zoho-test-session", async (req, res) => {
    try {
      ...
      console.log('[ZohoTestSession] 1. Salvando sessÃ£o:', { sessionId: testSessionId, data: testData });
      await savePendingZohoOAuth(testSessionId, testData);
      console.log('[ZohoTestSession] 2. SessÃ£o salva');

      console.log('[ZohoTestSession] 3. Recuperando sessÃ£o...');
      const retrieved = await getPendingZohoOAuth(testSessionId);
      console.log('[ZohoTestSession] 4. SessÃ£o recuperada:', retrieved ? 'SIM' : 'NÃƒO');

      if (!retrieved) {
        res.json({
          ok: false,
          step: 'retrieve',
          message: 'Falha ao recuperar sessÃ£o apÃ³s salvar',
      ...
      res.json({
        ok: true,
        message: 'SessÃ£o salva e recuperada com sucesso',
        sessionId: testSessionId,
        dataSaved: testData,
        dataRetrieved: retrieved,
      });
  ...
  // Retorna manifest.json com o Ã­cone atual configurado pelo ADM
  app.get("/manifest.json", async (_req, res) => {
    try {
      const loginImageUrl = await getSetting("login_image_url");
      // Usa a imagem do ADM se disponÃ­vel, senÃ£o usa os Ã­cones estÃ¡ticos
      ...
      const manifest = {
        ...
        name: appName,
        short_name: appName,
        description: "Atendimento rÃ¡pido para motoristas de app - Uber, 99 e InDrive",
        start_url: "/",
        scope: "/",
        display: "standalone",
      ...
      // Fallback: serve o manifest estÃ¡tico
