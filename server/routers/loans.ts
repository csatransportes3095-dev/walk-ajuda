...
// ─── Helper: obter data de hoje em UTC-3 (Brasil) ─────────────────────────────
...
async function generateReceiptPdf(data: {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor('#22c55e').fontSize(22).font('Helvetica-Bold').text('WALK AJUDA', 50, 28, { align: 'left' });
    ...
    doc.fillColor('#15803d').fontSize(12).font('Helvetica-Bold').text(`Nº do Recibo: ${data.receiptNumber}`, 60, recY + 10);
    doc.moveDown(1.5);
    ...
    doc.fillColor('#94a3b8').fontSize(8).font('Helvetica')
      .text('Este recibo é um comprovante de pagamento emitido pela Walk Ajuda.', 50, footerY + 10, { align: 'center', width: doc.page.width - 100 })
      .text('Guarde este documento para sua segurança.', 50, footerY + 22, { align: 'center', width: doc.page.width - 100 });
    doc.end();
  });
}

// ─── Helper: converter PDF de recibo para JPG ────────────────────────────────
...
export const loanRouter = router({
  ...
  // ── BUSCA CLIENTE NO SISTEMA PRINCIPAL ────────────────────────────────────
  ...
  sendRejectionNotice: adminProcedure.input(z.object({
    ...
    if (clientEmail) {
      try {
        ...
        await transporter.sendMail({
          from: '"CSA Empréstimos SP" <walkajuda@walkajuda.com>',
          to: clientEmail,
  ...
  confirmInstallmentPayment: adminProcedure.input(z.object({
    ...
    if (inst.length) {
      ...
      if (parseInt(pending[0].cnt) === 0) {
        // Todas as parcelas pagas → empréstimo pago
        await db.execute(drizzleSql`UPDATE loans SET status='pago', paidAt=NOW(), paidBy=${paidBy} WHERE id=${loanId}`);
      } else {
        // Ainda há parcelas pendentes → verificar se há alguma aguardando análise
        ...
        if (parseInt(awaitingProof[0].cnt) > 0) {
          // Ainda tem comprovante aguardando → mantém aguardando_pagamento
          await db.execute(drizzleSql`UPDATE loans SET status='aguardando_pagamento' WHERE id=${loanId}`);
        } else {
          // Nenhum comprovante pendente → volta para ativo/aprovado
  ...
  // ─── COMPROVANTES DE PAGAMENTO (ADMIN) ───────────────────────────────────────
  ...
  // ── EXTRATO COMPLETO DO EMPRÉSTIMO ──────────────────────────────────────────
      ...
      <div style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border:1.5px solid #fcd34d;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
        <table width="100%"><tr><td width="36" style="vertical-align:top;font-size:26px;">⭐</td><td>
          ...
          <div style="display:flex;align-items:center;gap:8px;">
            ...
            <span style="font-size:10px;color:#16a34a;font-weight:600;">↑ Limite até ${fmtBRL(parseFloat(nextProfile.creditLimit))} · Juros a partir de ${nextProfile.interestRate}%</span>
          </div>
        </td></tr></table>
      </div>
...
<div class="page">
  <div class="hdr">
    <div><h1 style="color:#f59e0b;font-size:20px;font-weight:800;margin:0;">CSA EMPRÉSTIMOS SP</h1><p style="color:#94a3b8;font-size:11px;margin:3px 0 0;">walkajuda.com</p></div>
  ...
  <div class="body">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:20px;">
      <div style="display:flex;justify-content:space-between;align-items:center;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:8px 12px;"><span style="font-size:10px;color:#64748b;">Nº do Empréstimo</span><span style="font-size:11px;font-weight:700;">#EMP-${String(input.loanId).padStart(4,'0')}</span></div>
  ...
  <div class="footer">
    <div><p style="font-size:9px;color:#94a3b8;">CSA Empréstimos SP · walkajuda.com · walkajuda@walkajuda.com</p><p style="font-size:9px;color:#94a3b8;margin-top:3px;">Documento gerado automaticamente. Válido como comprovante de situação do empréstimo.</p></div>
  ...
  // ── SYNC: Sincronizar clientes de empréstimo com senhas ativas do Gastos ──────
