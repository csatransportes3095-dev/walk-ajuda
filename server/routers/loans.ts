...
// â”€â”€â”€ Helper: obter data de hoje em UTC-3 (Brasil) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Helper: gerar PDF de recibo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function generateReceiptPdf(data: {
  ...
  isInterestOnly?: boolean; // true = recibo de juros (rolagem de dÃ­vida)
}): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const rows: string[][] = [
      ...
      ['CPF:', data.clientCpf || 'NÃ£o informado'],
    ...
    if (data.isInterestOnly) {
      ...
      rows.push(['Tipo:', 'Pagamento de Juros (DÃ­vida Rolada)']);
    ...
    rows.push(['PrÃ³ximo Vencimento:', data.nextDueDate || 'â€”']);
    rows.push(['Confirmado por:', data.confirmedBy || 'â€”']);
    let y = doc.y;
    ...
    doc.fillColor('#94a3b8').fontSize(8).font('Helvetica')
      .text('Este recibo Ã© um comprovante de pagamento emitido pela H2 COLOMBIANO.', 50, footerY + 10, { align: 'center', width: doc.page.width - 100 })
      .text('Guarde este documento para sua seguranÃ§a.', 50, footerY + 22, { align: 'center', width: doc.page.width - 100 });
    doc.end();
  });
}

// â”€â”€â”€ Helper: converter PDF de recibo para JPG â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function generateReceiptJpg(pdfBuffer: Buffer): Promise<Buffer> {
  ...
  try {
    ...
    // pdftoppm converte PDF para PPM (formato intermediÃ¡rio), depois sharp converte para JPG
    execSync(`pdftoppm -r 200 -jpeg -jpegopt quality=95 "${pdfPath}" "${jpgBase}"`, { timeout: 15000 });
    // Pegar o primeiro arquivo gerado (pÃ¡gina 1)
    ...
    if (!files.length) {
      ...
      if (!ppmFiles.length) throw new Error('pdftoppm nÃ£o gerou arquivo de saÃ­da');
      ppmFiles.sort();
...
async function sendReceiptEmail(to: string, clientName: string, receiptNumber: string, installmentNumber: number, pdfBuffer: Buffer): Promise<void> {
  ...
  await transporter.sendMail({
    ...
    to,
    subject: `Recibo de Pagamento â€” Parcela #${installmentNumber} | ${receiptNumber}`,
    html: `<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;"><div style="background:#0f172a;padding:24px;border-radius:8px 8px 0 0;"><h2 style="color:#22c55e;margin:0;">H2 COLOMBIANO</h2><p style="color:#94a3b8;margin:4px 0 0;">Recibo de Pagamento</p></div><div style="background:#f8fafc;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e2e8f0;"><p style="color:#0f172a;">OlÃ¡, <strong>${clientName}</strong>!</p><p style="color:#374151;">Segue em anexo o recibo da <strong>Parcela #${installmentNumber}</strong>.</p><p style="color:#374151;">NÃºmero do recibo: <strong>${receiptNumber}</strong></p><hr style="border:none;border-top:1px solid #e2e8f0;margin:16px 0;"><p style="color:#6b7280;font-size:12px;">Guarde este recibo para sua seguranÃ§a.</p></div></div>`,
...
// AvanÃ§a para o prÃ³ximo dia Ãºtil conforme workDays ("seg_sab" = 0=dom excluÃ­do, "seg_dom" = todos)
...
// NÃºmero fixo de parcelas diÃ¡rias:
// Segâ€“SÃ¡b = 20 parcelas | Segâ€“Dom = 25 parcelas | custom = definido pelo admin
...
// Calcula simulaÃ§Ã£o de parcelas sem criar no banco (usado pelo frontend para preview)
...
export const loanRouter = router({
  ...
  // LADO DO ADMIN â€” adminProcedure
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

  // â”€â”€ BUSCA CLIENTE NO SISTEMA PRINCIPAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ PIX CONFIG â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ PERFIS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  syncProfile: adminProcedure.input(z.object({
    ...
    if (!profiles.length) throw new Error("Perfil nÃ£o encontrado");
    const p = profiles[0];
  ...
  // â”€â”€ CLIENTES (admin) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ EMPRÃ‰STIMOS (admin) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  listLoans: adminProcedure.input(z.object({
    ...
    // Busca parcelas que vencem hoje para cada emprÃ©stimo aprovado
    ...
    // Busca emprÃ©stimos que tÃªm QUALQUER parcela pendente com vencimento anterior a hoje
    ...
    if (input?.status && input.status !== 'todos') {
      if (input.status === 'ativos') {
        ...
        // Inclui emprÃ©stimos com status aguardando_pagamento OU aprovado com parcela vencendo hoje
        result = result.filter((r: any) => r.status === 'aguardando_pagamento' || r.hasInstallmentDueToday);
      } else if (input.status === 'em_analise') {
        // EmprÃ©stimos que tÃªm pelo menos uma parcela em anÃ¡lise (comprovante enviado pelo cliente)
        result = result.filter((r: any) => Number(r.pendingProofs) > 0);
      } else if (input.status === 'pago_hoje') {
        // EmprÃ©stimos com pelo menos uma parcela paga hoje (convertendo paidAt para BRT)
  ...
  createLoan: adminProcedure.input(z.object({
    ...
    if (!clients.length) throw new TRPCError({ code: "NOT_FOUND", message: "Cliente nÃ£o encontrado" });
  ...
  sendRejectionNotice: adminProcedure.input(z.object({
    ...
    const db = await getDb() as any;
    // Buscar dados do emprÃ©stimo e cliente
    ...
    `);
    if (!loanRows.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'EmprÃ©stimo nÃ£o encontrado' });
    ...
    if (clientEmail) {
      try {
        ...
        await transporter.sendMail({
          from: '"CSA EmprÃ©stimos SP" <h2@h2colombiano.com>',
          to: clientEmail,
          subject: `AtualizaÃ§Ã£o sobre sua solicitaÃ§Ã£o de emprÃ©stimo â€” CSA EmprÃ©stimos SP`,
          html: `<div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;">
            <div style="background:#0f172a;padding:24px;border-radius:8px 8px 0 0;">
              <h2 style="color:#ef4444;margin:0;">CSA EmprÃ©stimos SP</h2>
              <p style="color:#94a3b8;margin:4px 0 0;">Resultado da AnÃ¡lise de CrÃ©dito</p>
            </div>
            <div style="background:#f8fafc;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e2e8f0;">
              <p style="color:#0f172a;">OlÃ¡, <strong>${loan.clientName}</strong>!</p>
              <p style="color:#374151;">Informamos que sua solicitaÃ§Ã£o de emprÃ©stimo no valor de <strong>${amountFmt}</strong> foi <strong style="color:#ef4444;">reprovada</strong>.</p>
              ...
              <p style="color:#374151;">Em caso de dÃºvidas, entre em contato conosco pelo WhatsApp.</p>
              <hr style="border:none;border-top:1px solid #e2e8f0;margin:16px 0;">
              <p style="color:#6b7280;font-size:12px;">CSA EmprÃ©stimos SP â€” Atendimento ao cliente</p>
            </div>
          </div>`,
        });
        sentTo = clientEmail;
    ...
    const waMsg = `OlÃ¡ ${loan.clientName}! Infelizmente sua solicitaÃ§Ã£o de emprÃ©stimo foi reprovada.\n\nMotivo: ${input.reason}\n\nEm caso de dÃºvidas, entre em contato conosco.`;
  ...
  confirmInstallmentPayment: adminProcedure.input(z.object({
    ...
    if (inst.length) {
      ...
      if (parseInt(pending[0].cnt) === 0) {
        // Todas as parcelas pagas â†’ emprÃ©stimo pago
        await db.execute(drizzleSql`UPDATE loans SET status='pago', paidAt=NOW(), paidBy=${paidBy} WHERE id=${loanId}`);
      } else {
        // Ainda hÃ¡ parcelas pendentes â†’ verificar se hÃ¡ alguma aguardando anÃ¡lise
        ...
        if (parseInt(awaitingProof[0].cnt) > 0) {
          // Ainda tem comprovante aguardando â†’ mantÃ©m aguardando_pagamento
          await db.execute(drizzleSql`UPDATE loans SET status='aguardando_pagamento' WHERE id=${loanId}`);
        } else {
          // Nenhum comprovante pendente â†’ volta para ativo/aprovado
  ...
  // Desfaz pagamento confirmado: volta para pendente, MANTÃ‰M comprovante para consulta
  undoInstallmentPayment: adminProcedure.input(z.object({
    ...
    const loanId = inst[0].loanId;
    // Volta para pendente mas MANTÃ‰M proofUrl e proofSentAt para consulta futura
    ...
    // Se o emprÃ©stimo estava marcado como pago, volta para aprovado
  ...
  // â”€â”€ DASHBOARD (admin) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  getDashboard: adminProcedure.query(async () => {
    const db = await getDb() as any;
    // Usar fuso horÃ¡rio de SÃ£o Paulo (GMT-3) para calcular "hoje"
    const today = getBrazilToday();

    // Totais gerais dos emprÃ©stimos
    ...
    // Total previsto = soma de TODAS as parcelas de emprÃ©stimos ativos (capital + juros)
    ...
    // Total recebido = soma das parcelas individualmente pagas (independente do status do emprÃ©stimo)
    ...
    // Juros recebidos: proporcional ao valor pago em relaÃ§Ã£o ao total do emprÃ©stimo
    ...
    // Lucro Total Previsto = interestAmount total de todos os emprÃ©stimos ativos
    ...
    // Capital Recuperado = do total recebido, quanto Ã© capital (nÃ£o juros)
    ...
    // GrÃ¡fico mensal: dois datasets separados
    // 1) Emprestado por mÃªs de criaÃ§Ã£o do emprÃ©stimo
    ...
    `);
    // 2) Recebido por mÃªs em que a parcela foi efetivamente paga
    const receivedByMonth = await qRows(db, drizzleSql`
      ...
      GROUP BY month ORDER BY month ASC
    `);
    // Mescla os dois datasets pelo mÃªs
  ...
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  // LADO DO CLIENTE â€” via token da planilha (publicProcedure)
  // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

  getClientLoanInfo: publicProcedure.input(z.object({ token: z.string() })).query(async ({ input }) => {
    const db = await getDb() as any;
    const token = input.token.trim();
    // Buscar sessÃ£o da planilha para obter dados do cliente
    ...
    if (!clients.length) {
      // Verificar se jÃ¡ existe um loanClient com o mesmo CPF ou telefone (evitar duplicidade)
      ...
      if (existingByIdentity.length) {
        ...
        // Criar automaticamente com loanEnabled=1 (padrÃ£o liberado)
    ...
    `);
    const today = getBrazilToday();
    // Busca emprÃ©stimos com parcelas pendentes vencidas
  ...
  // SimulaÃ§Ã£o de parcelas (preview antes de confirmar) â€” nÃ£o cria no banco
  simulateLoan: publicProcedure.input(z.object({
    ...
    if (!clients.length) throw new TRPCError({ code: "UNAUTHORIZED", message: "EmprÃ©stimos nÃ£o habilitados para este cliente" });
    const client = clients[0];
    ...
    // NÃ£o expor taxa ao cliente â€” retorna apenas o resultado do cÃ¡lculo
    return {
  ...
  requestLoan: publicProcedure.input(z.object({
    ...
    if (!client.client_pix_key || !client.client_pix_name || !client.client_pix_bank) {
      throw new TRPCError({ code: "BAD_REQUEST", message: "VocÃª precisa cadastrar sua chave PIX completa (chave, nome do titular e banco) antes de solicitar um emprÃ©stimo." });
    }

    // Valida se o tipo de pagamento estÃ¡ liberado para este cliente
    const allowed = (client.allowedPaymentTypes || "diario").split(",").map((t: string) => t.trim());
    if (!allowed.includes(input.paymentType)) {
      throw new TRPCError({ code: "BAD_REQUEST", message: `Modo de pagamento "${input.paymentType}" nÃ£o liberado para este cliente` });
    }
  ...
  saveClientPixKey: publicProcedure.input(z.object({
    token: z.string(),
    pixKey: z.string().min(5, "Chave PIX invÃ¡lida").max(200),
    pixName: z.string().min(2, "Nome do titular obrigatÃ³rio").max(200),
    pixBank: z.string().min(2, "Nome do banco obrigatÃ³rio").max(100),
  })).mutation(async ({ input }) => {
    const db = await getDb() as any;
    const token = input.token.trim();
    const clients = await qRows(db, drizzleSql`SELECT * FROM loanClients WHERE spreadsheetToken=${token}`);
  ...
  // Lista todos os clientes com senha no /gastos com status de emprÃ©stimo habilitado
  ...
  // â”€â”€ TAXA DE ATRASO E REGRAS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

  // Busca configuraÃ§Ã£o de taxa de atraso (pÃºblica â€” usada pelo cliente para exibir regras)
  ...
  // Salva configuraÃ§Ã£o de taxa de atraso (admin)
  ...
  // Toggle taxa de atraso por cliente especÃ­fico (admin)
  ...
  // Calcula taxa de atraso para uma parcela especÃ­fica (chamado pelo cliente ao ver parcelas)
  calcLateFee: publicProcedure.input(z.object({
    ...
    // SÃ³ aplica taxa se a parcela estÃ¡ pendente/atrasada e Ã© do dia de hoje
    const today = getBrazilToday();
    ...
    if (nowHour >= 18 && nowHour < 20) {
      ...
      breakdown = [`ApÃ³s 18h: +R$ ${lateFee.toFixed(2)}`];
    } else if (nowHour >= 20 && nowHour < 24) {
      lateFee = parseFloat(cfg.fee_after_18h) + parseFloat(cfg.fee_after_20h);
      breakdown = [`ApÃ³s 18h: +R$ ${parseFloat(cfg.fee_after_18h).toFixed(2)}`, `ApÃ³s 20h: +R$ ${parseFloat(cfg.fee_after_20h).toFixed(2)}`];
    } else if (nowHour >= 0 && installment.dueDate < today) {
      // Passou da meia-noite â€” 100% da parcela
      const pct = parseFloat(cfg.fee_after_midnight_pct) / 100;
      lateFee = Math.round(amount * pct * 100) / 100;
      breakdown = [`ApÃ³s 23:59: +${cfg.fee_after_midnight_pct}% da parcela (R$ ${lateFee.toFixed(2)})`];
    }

    return { lateFee, totalWithFee: amount + lateFee, breakdown };
  }),

  // Ativar/desativar emprÃ©stimo por telefone (cria loanClient se nÃ£o existir)
  toggleLoanByPhone: adminProcedure.input(z.object({
    ...
    if (existing.length) {
      // Atualiza todos os registros com esse telefone (evita inconsistÃªncia em duplicatas)
  ...
  // â”€â”€ ANÃLISE FINANCEIRA (admin) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  getFinancialAnalysis: adminProcedure.input(z.object({
    ...
    // Montar condiÃ§Ã£o de data para parcelas
    ...
    // 1. JÃ¡ recebi (parcelas pagas no perÃ­odo)
    ...
    // 2. Vou receber (parcelas pendentes com vencimento no perÃ­odo)
    ...
    // 3. Quanto vou ganhar de juros (proporcional ao perÃ­odo)
    ...
    // 4. Juros jÃ¡ recebidos no perÃ­odo
    ...
    // 5. Perdas: emprestado em emprÃ©stimos cancelados/reprovados no perÃ­odo
    ...
    // 6. InadimplÃªncia: parcelas atrasadas (vencidas e nÃ£o pagas) no perÃ­odo
    const today = getBrazilToday();
    ...
    // 7. Timeline: parcelas a receber por dia (para grÃ¡fico)
    ...
    // 8. PrÃ³ximas parcelas a vencer (para tabela "quando vou receber")
  ...
  // Corrige parcelas pendentes que caem no domingo para emprÃ©stimos com workDays=seg_sab
  fixSundayInstallments: adminProcedure.mutation(async () => {
    const db = await getDb() as any;
    // Busca parcelas pendentes em domingo de emprÃ©stimos seg_sab
    ...
    // Atualiza o dueDate do emprÃ©stimo (data da Ãºltima parcela)
  ...
  // Edita um emprÃ©stimo ativo: recalcula juros, total e parcelas pendentes
  editLoan: adminProcedure.input(z.object({
    ...
    if (!loans.length) throw new TRPCError({ code: "NOT_FOUND", message: "EmprÃ©stimo nÃ£o encontrado" });
    const loan = loans[0];

    // Recalcula com os novos valores
    const sim = simulateLoan(input.amount, input.interestRate, input.paymentType, input.days, input.workDays, input.releaseDate, input.customInstallments);

    // Descobre quantas parcelas jÃ¡ foram pagas
  ...
  // â”€â”€ Pagamento SÃ³ de Juros â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  payInterestOnly: adminProcedure.input(z.object({
    loanId: z.number(),
    installmentId: z.number(), // ID da parcela vencida especÃ­fica
  })).mutation(async ({ ctx, input }) => {
    const db = await getDb() as any;
    const loans = await qRows(db, drizzleSql`SELECT * FROM loans WHERE id=${input.loanId}`);
    if (!loans.length) throw new TRPCError({ code: "NOT_FOUND", message: "EmprÃ©stimo nÃ£o encontrado" });
    const loan = loans[0];
    if (!loan.interestOnlyEnabled) throw new TRPCError({ code: "BAD_REQUEST", message: "FunÃ§Ã£o de pagamento sÃ³ de juros nÃ£o estÃ¡ ativada neste emprÃ©stimo" });
    if (["pago", "cancelado", "reprovado"].includes(loan.status)) throw new TRPCError({ code: "BAD_REQUEST", message: "EmprÃ©stimo jÃ¡ encerrado" });

    // Busca a parcela especÃ­fica
    const today = getBrazilToday();
    ...
    if (!instRows.length) throw new TRPCError({ code: "NOT_FOUND", message: "Parcela nÃ£o encontrada" });
    const inst = instRows[0];
    if (inst.status !== 'pendente') throw new TRPCError({ code: "BAD_REQUEST", message: "Parcela nÃ£o estÃ¡ pendente" });
    if (inst.dueDate >= today) throw new TRPCError({ code: "BAD_REQUEST", message: "Parcela ainda nÃ£o venceu" });
    ...
    // Principal desta parcela = valor total do emprÃ©stimo / nÃºmero de parcelas
    ...
    // Busca a Ãºltima data de vencimento existente entre as parcelas pendentes (excluindo a atual)
    // para criar a nova parcela DEPOIS da Ãºltima
    ...
    // Novo amount da parcela rolada = principal + juros novos (sem a taxa, que jÃ¡ foi cobrada)
    const newInstAmount = Math.round((principalPerInstallment + interestOnPrincipal) * 100) / 100;

    // PrÃ³ximo nÃºmero de parcela (apÃ³s o maior existente)
    ...
    // 2. Cria nova parcela com principal + juros novos (dÃ­vida rolada), vencendo apÃ³s a Ãºltima
    await db.execute(drizzleSql`
    ...
    // 3. Atualiza vencimento do emprÃ©stimo para a nova data e incrementa contador
    await db.execute(drizzleSql`
    ...
    try {
      ...
      await createFinancialSale({
        ...
        productName: `Juros EmprÃ©stimo #EMP-${String(input.loanId).padStart(4,'0')}`,
        productOption: `Parcela #${inst.installmentNumber} â€” DÃ­vida rolada`,
  ...
  // â”€â”€â”€ DESFAZER PAGAMENTO DE JUROS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  undoInterestOnly: adminProcedure.input(z.object({
    ...
    if (!instRows.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Parcela nÃ£o encontrada' });
    const inst = instRows[0];
    if (inst.status !== 'pago_juros') throw new TRPCError({ code: 'BAD_REQUEST', message: 'Parcela nÃ£o estÃ¡ com status pago_juros' });
    ...
    `);
    // 3. Deleta a parcela rolada (rolled_from_interest_only) criada apÃ³s esta
    // Busca a parcela com nÃºmero maior que a atual e notes contendo rolled_from_interest_only
    await db.execute(drizzleSql`
    ...
    `);
    // 4. Restaura o vencimento do emprÃ©stimo para a Ãºltima parcela pendente restante
  ...
  // â”€â”€â”€ COMPROVANTES DE PAGAMENTO (ADMIN) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // Buscar comprovantes de um emprÃ©stimo (batch por loanId)
  ...
  // Adicionar comprovante a pagamento jÃ¡ confirmado sem arquivo
  ...
  replaceInstallmentProof: adminProcedure.input(z.object({
    ...
    const inst = instRows[0];
    const existing = await qRows(db, drizzleSql`SELECT * FROM installmentProofs WHERE installmentId=${input.installmentId} ORDER BY id DESC LIMIT 1`);
    if (!existing.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Comprovante nÃ£o encontrado' });
  ...
  // Excluir comprovante (nÃ£o desfaz pagamento)
  ...
  // HistÃ³rico de comprovantes com filtros
  ...
  // â”€â”€â”€ RECIBO DE PAGAMENTO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ EXTRATO COMPLETO DO EMPRÃ‰STIMO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  generateLoanStatement: adminProcedure.input(z.object({
    loanId: z.number(),
  })).mutation(async ({ input }) => {
    console.log('[generateLoanStatement] START loanId:', input.loanId);
    const db = await getDb() as any;
    // Buscar dados do emprÃ©stimo + cliente
    ...
    `);
    // Buscar perfil atual e prÃ³ximo
    ...
    const cpfFmt = rawCpf.length === 11 ? `${rawCpf.slice(0,3)}.${rawCpf.slice(3,6)}.${rawCpf.slice(6,9)}-${rawCpf.slice(9)}` : (rawCpf || 'NÃ£o informado');
    const fmtBRL = (v: number) => v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    const fmtDate = (d: string) => d ? new Date(d + 'T12:00:00Z').toLocaleDateString('pt-BR') : 'â€”';
    const statusLabel: Record<string, string> = { aprovado: 'Aprovado', pendente: 'Aguardando AprovaÃ§Ã£o', reprovado: 'Reprovado', cancelado: 'Cancelado', pago: 'Pago' };
    const payTypeLabel: Record<string, string> = { diario: 'DiÃ¡rio', semanal: 'Semanal', quinzenal: 'Quinzenal', mensal: 'Mensal' };
    const profileEmoji: Record<string, string> = { bronze: 'ðŸ¥‰', prata: 'â­', ouro: 'ðŸ¥‡', diamante: 'ðŸ’Ž' };
    ...
    const installmentsRows = installments.map((inst: any) => {
      ...
      const pillLabel: Record<string, string> = { pago: 'âœ“ Pago', pendente: 'â³ Pendente', atrasado: 'âš  Atrasado', em_analise: 'ðŸ” Em AnÃ¡lise', cancelado: 'Cancelado' };
      const pStyle = pillColor[statusKey] || pillColor.pendente;
      const pLabel = pillLabel[statusKey] || inst.status;
      const paidAtStr = inst.paidAt ? fmtDate(inst.paidAt.toString().slice(0, 10)) : 'â€”';
      return `<tr>
    ...
    const upgradeSection = nextProfile ? `
      <div style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border:1.5px solid #fcd34d;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
        <table width="100%"><tr><td width="36" style="vertical-align:top;font-size:26px;">â­</td><td>
          <p style="font-size:13px;font-weight:700;color:#92400e;margin:0 0 6px;">ParabÃ©ns! VocÃª estÃ¡ no caminho certo, ${loan.clientName.split(' ')[0]}!</p>
          <p style="font-size:11px;color:#78350f;line-height:1.6;margin:0 0 10px;">VocÃª estÃ¡ no perfil <strong>${currentProfile?.name || loan.profileSlug}</strong> com limite de <strong>${fmtBRL(parseFloat(loan.clientCreditLimit || 0))}</strong>. Mantendo seus pagamentos em dia, vocÃª sobe de categoria <strong>automaticamente</strong> e ganha acesso a limites maiores e taxas menores!</p>
          <div style="display:flex;align-items:center;gap:8px;">
            <span style="background:#e2e8f0;color:#475569;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;">${profileEmoji[loan.profileSlug] || 'â­'} ${currentProfile?.name || loan.profileSlug}</span>
            <span style="color:#d97706;font-size:14px;">â†’</span>
            <span style="background:#fef3c7;color:#92400e;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;border:1px solid #fcd34d;">${profileEmoji[nextProfile.slug] || 'ðŸ¥‡'} ${nextProfile.name}</span>
            <span style="font-size:10px;color:#16a34a;font-weight:600;">â†‘ Limite atÃ© ${fmtBRL(parseFloat(nextProfile.creditLimit))} Â· Juros a partir de ${nextProfile.interestRate}%</span>
          </div>
        </td></tr></table>
      </div>
      <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:14px 18px;margin-bottom:20px;">
        <p style="font-size:11px;font-weight:700;color:#15803d;margin:0 0 8px;">âœ… Como subir de categoria automaticamente:</p>
        <ul style="font-size:10px;color:#166534;line-height:1.8;padding-left:16px;margin:0;">
          ...
          <li>Mantenha bom histÃ³rico por pelo menos 2 emprÃ©stimos consecutivos</li>
          <li>NÃ£o tenha parcelas em aberto vencidas</li>
          <li>A promoÃ§Ã£o de categoria Ã© feita automaticamente pelo sistema</li>
        </ul>
      </div>` : `
      <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:14px 18px;margin-bottom:20px;">
        <p style="font-size:12px;font-weight:700;color:#15803d;margin:0 0 4px;">ðŸ† VocÃª estÃ¡ no nÃ­vel mÃ¡ximo!</p>
        <p style="font-size:11px;color:#166534;margin:0;">Continue mantendo seus pagamentos em dia para preservar seus benefÃ­cios exclusivos.</p>
      </div>`;
...
<div class="page">
  <div class="hdr">
    <div><h1 style="color:#f59e0b;font-size:20px;font-weight:800;margin:0;">CSA EMPRÃ‰STIMOS SP</h1><p style="color:#94a3b8;font-size:11px;margin:3px 0 0;">h2colombiano.com</p></div>
    <div style="text-align:right;"><div style="color:#fff;font-size:13px;font-weight:600;background:rgba(245,158,11,.15);border:1px solid rgba(245,158,11,.3);padding:4px 12px;border-radius:20px;">ðŸ“„ EXTRATO DE EMPRÃ‰STIMO</div><div style="color:#64748b;font-size:10px;margin-top:5px;">Emitido em: ${emittedAt}</div></div>
  </div>
  <div class="body">
    <div class="sec">ðŸ‘¤ Dados do Cliente</div>
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:16px 20px;margin-bottom:20px;display:flex;align-items:center;gap:16px;">
      ...
      <div style="flex:1;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:5px;">
          ${loan.clientPhone ? `<span style="font-size:11px;color:#475569;">ðŸ“± ${loan.clientPhone}</span>` : ''}
          ${cpfFmt !== 'NÃ£o informado' ? `<span style="font-size:11px;color:#475569;">ðŸªª CPF: ${cpfFmt}</span>` : ''}
          ${loan.clientEmail ? `<span style="font-size:11px;color:#475569;">ðŸ“§ ${loan.clientEmail}</span>` : ''}
        </div>
      </div>
      <div style="text-align:center;flex-shrink:0;">
        <div style="display:inline-block;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:700;background:#e2e8f0;color:#475569;border:1px solid #cbd5e1;">${profileEmoji[loan.profileSlug] || 'â­'} ${(currentProfile?.name || loan.profileSlug).toUpperCase()}</div>
    ...
    <div class="sec">ðŸ’° Resumo Financeiro</div>
    ...
    <div style="margin-bottom:20px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;"><span style="font-size:11px;color:#475569;font-weight:500;">Progresso do pagamento</span><strong style="font-size:12px;color:#0f172a;">${progressPct}% concluÃ­do</strong></div>
    ...
    <div class="sec">ðŸ“‹ Detalhes do EmprÃ©stimo</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:20px;">
      <div style="display:flex;justify-content:space-between;align-items:center;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:8px 12px;"><span style="font-size:10px;color:#64748b;">NÂº do EmprÃ©stimo</span><span style="font-size:11px;font-weight:700;">#EMP-${String(input.loanId).padStart(4,'0')}</span></div>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:8px 12px;"><span style="font-size:10px;color:#64748b;">Data de LiberaÃ§Ã£o</span><span style="font-size:11px;font-weight:700;">${fmtDate(loan.releaseDate)}</span></div>
      ...
      ${loan.notes ? `<div style="display:flex;justify-content:space-between;align-items:center;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:8px 12px;"><span style="font-size:10px;color:#64748b;">ObservaÃ§Ãµes</span><span style="font-size:11px;font-weight:700;">${loan.notes}</span></div>` : ''}
    </div>
    <div class="sec">ðŸ“… HistÃ³rico de Parcelas</div>
    <table class="inst">
    ...
    <div class="sec">ðŸš€ Seu Perfil &amp; PrÃ³ximo NÃ­vel</div>
    ${upgradeSection}
  </div>
  <div class="footer">
    <div><p style="font-size:9px;color:#94a3b8;">CSA EmprÃ©stimos SP Â· h2colombiano.com Â· h2@h2colombiano.com</p><p style="font-size:9px;color:#94a3b8;margin-top:3px;">Documento gerado automaticamente. VÃ¡lido como comprovante de situaÃ§Ã£o do emprÃ©stimo.</p></div>
    ...
    // Gerar PDF via weasyprint (disponÃ­vel no servidor)
    ...
    try {
      ...
      if (!fs.existsSync(pdfPath)) {
        throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'PDF nÃ£o foi gerado pelo weasyprint' });
      }
    ...
    const phone = loan.clientPhone ? loan.clientPhone.replace(/\D/g, '') : null;
    const waMsg = `OlÃ¡ ${loan.clientName}! Segue o extrato completo do seu emprÃ©stimo #EMP-${String(input.loanId).padStart(4,'0')}.\n\nValor: ${fmtBRL(parseFloat(loan.amount))} | Total c/ juros: ${fmtBRL(totalAmount)}\nPago: ${fmtBRL(totalPaid)} | Saldo: ${fmtBRL(remaining)}\n\nAcesse o PDF: ${url}\n\nEm caso de dÃºvidas, entre em contato conosco.`;
    const whatsappUrl = phone ? `https://wa.me/55${phone}?text=${encodeURIComponent(waMsg)}` : null;
  ...
  sendLoanStatementByEmail: adminProcedure.input(z.object({
    ...
    const db = await getDb() as any;
    // Buscar e-mail se nÃ£o fornecido
    ...
    if (!toEmail) throw new TRPCError({ code: 'BAD_REQUEST', message: 'E-mail do cliente nÃ£o encontrado. Informe o e-mail manualmente.' });
    ...
    await transporter.sendMail({
      from: '"CSA EmprÃ©stimos SP" <h2@h2colombiano.com>',
      to: toEmail,
      subject: `Extrato do seu EmprÃ©stimo â€” ${input.docId} | CSA EmprÃ©stimos SP`,
      html: `<div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;"><div style="background:#0f172a;padding:24px;border-radius:8px 8px 0 0;"><h2 style="color:#f59e0b;margin:0;">CSA EmprÃ©stimos SP</h2><p style="color:#94a3b8;margin:4px 0 0;">Extrato de EmprÃ©stimo</p></div><div style="background:#f8fafc;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e2e8f0;"><p style="color:#0f172a;">OlÃ¡, <strong>${input.clientName}</strong>!</p><p style="color:#374151;">Segue em anexo o extrato completo do seu emprÃ©stimo <strong>${input.docId}</strong>.</p><p style="color:#374151;">O documento contÃ©m todas as informaÃ§Ãµes do emprÃ©stimo, histÃ³rico de parcelas e detalhes do seu perfil de crÃ©dito.</p><hr style="border:none;border-top:1px solid #e2e8f0;margin:16px 0;"><p style="color:#6b7280;font-size:12px;">CSA EmprÃ©stimos SP â€” Atendimento ao cliente</p></div></div>`,
  ...
  // Aplica taxa de atraso prÃ©-estabelecida a uma parcela (admin)
  applyLateFeeToInstallment: adminProcedure.input(z.object({
    ...
    if (!inst.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Parcela nÃ£o encontrada' });
    const current = inst[0];
    if (current.status === 'pago') throw new TRPCError({ code: 'BAD_REQUEST', message: 'Parcela jÃ¡ estÃ¡ paga' });
    // Salva o valor original se ainda nÃ£o foi salvo
  ...
  // Gerar PDF de oferta de emprÃ©stimo prÃ©-aprovado e retornar link WhatsApp
  generateLoanOffer: adminProcedure.input(z.object({
    ...
    const { url } = await storagePut(fileKey, pdfBuffer, 'application/pdf');
    const paymentTypeLabel = input.paymentType === 'diario' ? 'DiÃ¡rio' : input.paymentType === 'semanal' ? 'Semanal' : input.paymentType === 'quinzenal' ? 'Quinzenal' : 'Mensal';
  ...
  // Reagenda parcelas pendentes de um emprÃ©stimo diÃ¡rio para novo regime (seg_sab ou seg_dom)
  rescheduleInstallments: adminProcedure.input(z.object({
    ...
    if (!loanRows.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'EmprÃ©stimo nÃ£o encontrado' });
    const loan = loanRows[0];
    if (loan.paymentType !== 'diario') throw new TRPCError({ code: 'BAD_REQUEST', message: 'Reagendamento disponÃ­vel apenas para emprÃ©stimos diÃ¡rios' });
    // Buscar parcelas pendentes (nÃ£o pagas)
    const pendingRows = await qRows(db, drizzleSql`
      SELECT * FROM loanInstallments
      WHERE loanId=${input.loanId} AND status != 'pago'
      ORDER BY installmentNumber ASC
    `);
    if (!pendingRows.length) throw new TRPCError({ code: 'BAD_REQUEST', message: 'NÃ£o hÃ¡ parcelas pendentes para reagendar' });
    // Buscar Ãºltima parcela paga para usar como base de data
    const lastPaidRows = await qRows(db, drizzleSql`
      SELECT dueDate FROM loanInstallments
      WHERE loanId=${input.loanId} AND status='pago'
      ORDER BY installmentNumber DESC LIMIT 1
    `);
    // Data base: hoje ou data da Ãºltima parcela paga (o que for mais recente)
    ...
    // Atualizar workDays no emprÃ©stimo
  ...
  // â”€â”€ SYNC: Sincronizar clientes de emprÃ©stimo com senhas ativas do Gastos â”€â”€â”€â”€â”€â”€
  syncFromGastos: adminProcedure.mutation(async () => {
    ...
    // Inserir todos os clientes do Gastos que ainda nÃ£o existem no EmprÃ©stimos
