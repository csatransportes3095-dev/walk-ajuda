...
export const spreadsheetRouter = router({
      ...
      try {
        ...
        if (isCpf) {
          ...
          if (!client) {
            ...
            // Cliente existe em customers — criar registro em spreadsheetClients automaticamente
            const insertResult = await db.insert(spreadsheetClients).values({
  ...
  // Buscar cliente por telefone na walkajuda.com
  adminSearchCustomer: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        // Buscar cliente em customers (walkajuda.com) - usar db diretamente
        const customer = await db.select().from(customers).where(eq(customers.phone, normalizedPhone)).limit(1);

        if (!customer || customer.length === 0) {
          throw new TRPCError({ code: "NOT_FOUND", message: "Cliente não encontrado na walkajuda.com" });
        }
  ...
  adminGenerateTemporaryPassword: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        // Buscar cliente em customers (walkajuda.com)
  ...
  // Cliente cria senha automaticamente (modo auto) — 30 dias, sem pendingApproval
