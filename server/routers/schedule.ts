...
export const scheduleRouter = router({
  ...
  // Lista agendamentos de um pedido para a PÁGINA DE ACOMPANHAMENTO (público)
  ...
  // Lista agendamentos pelo TELEFONE para a PÁGINA DE ACOMPANHAMENTO (público).
    ...
    .mutation(async ({ input }) => {
      ...
      (async () => {
        ...
        if (orderEmail) {
          const html = `
            <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#fff;border-radius:12px">
              <h2 style="color:#16a34a;margin:0 0 12px">✅ Cliente agendou o atendimento</h2>
              <p style="margin:0 0 8px;font-size:14px;color:#333"><strong>Pedido:</strong> #${appt.registrationId}</p>
              <p style="margin:0 0 8px;font-size:14px;color:#333"><strong>Cliente:</strong> ${appt.customerName || ""} (${appt.customerPhone})</p>
              ${appt.serviceName ? `<p style=\"margin:0 0 8px;font-size:14px;color:#333\"><strong>Serviço:</strong> ${appt.serviceName}</p>` : ""}
              <p style="margin:8px 0 0;font-size:16px;color:#111"><strong>Data/Hora:</strong> ${appt.slotDate} às ${appt.slotTime}</p>
            </div>`;
    ...
    .mutation(async ({ input }) => {
      ...
      (async () => {
        ...
        if (orderEmail) {
          const html = `
            <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#fff;border-radius:12px">
              <h2 style="color:#d97706;margin:0 0 12px">⚠️ Cliente solicitou reagendamento</h2>
              <p style="margin:0 0 8px;font-size:14px;color:#333"><strong>Pedido:</strong> #${appt.registrationId}</p>
              <p style="margin:0 0 8px;font-size:14px;color:#333"><strong>Cliente:</strong> ${appt.customerName || ""} (${appt.customerPhone})</p>
