...
describe("H2 Ads base migration safety", () => {
  it("permite criar apenas tabelas H2 Ads de forma idempotente", () => {
    ...
    expect(() => assertH2AdsSchemaStatementSafe("CREATE TABLE IF NOT EXISTS `h2ads_groups` (updatedAt timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)")).not.toThrow();
  });
  ...
  it("rejeita tabelas fora do módulo e comandos de alteração de dados", () => {
    ...
    expect(() => assertH2AdsSchemaStatementSafe("CREATE TABLE IF NOT EXISTS `h2ads_groups` (id int); UPDATE h2ads_groups SET id = 1")).toThrow();
  });
});
