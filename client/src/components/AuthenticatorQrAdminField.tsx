...
import { ImagePlus, Maximize2, Trash2, Upload } from 'lucide-react';
    ...
    <div className="rounded-xl border border-lime-500/25 bg-lime-500/[0.035] p-3 space-y-2.5">
      ...
      {previewData ? (
        ...
        >
          ...
          <p className="text-xs font-semibold text-lime-200">Cole com CTRL+V, arraste uma imagem ou toque para selecionar</p>
          <p className="text-[10px] text-muted-foreground mt-1">PNG, JPG/JPEG ou WebP · até 3 MB</p>
          <button type="button" onClick={event => { event.stopPropagation(); fileInputRef.current?.click(); }} className="mt-2 rounded-lg border border-lime-400/35 bg-lime-500/10 px-3 py-1.5 text-[11px] font-bold text-lime-200 hover:bg-lime-500/20"><Upload className="inline w-3.5 h-3.5 mr-1" />Selecionar imagem</button>
        </div>
      )}
