...
export default function WelcomeScreen({ children }: { children: React.ReactNode }) {
  ...
  const renderSupportButton = () => {
    ...
    const supportSecondaryText = supportStatusText?.trim() || supportDescription;
    return (
      <button
        type="button"
        className="w-full text-left transition-transform duration-150 hover:-translate-y-0.5 active:scale-[0.985]"
        onClick={() => setOnlineSupportOpen(true)}
        aria-label={`Abrir ${supportLabel}`}
        style={elegantBtnStyle(supportColor, { width: "100%", minHeight: 84, display: "flex", alignItems: "center", gap: 14, textAlign: "left" })}
      >
        <span
          className="flex h-12 w-12 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl border border-white/20 bg-black/20 shadow-inner"
          aria-hidden="true"
        >
          {supportAvatar ? (
            <img src={supportAvatar} alt="" className="h-full w-full object-cover" />
          ) : (
