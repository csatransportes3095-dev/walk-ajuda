...
import { Clock3, LockKeyhole, Power, Save, Wrench } from "lucide-react";
    ...
    <section className="relative overflow-hidden rounded-3xl border border-cyan-300/20 bg-[#071326] p-5 shadow-[0_0_40px_rgba(34,211,238,.08)] sm:p-7">
      ...
      <div className="relative space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
          ...
          <aside className="rounded-2xl border border-rose-300/20 bg-rose-300/[0.06] p-4 sm:p-5">
            <div className="flex items-center gap-2"><LockKeyhole className="h-4 w-4 text-rose-200" /><h3 className="font-black text-white">Bloqueio total</h3></div>
            <p className="mt-2 text-xs leading-5 text-slate-300">Com o manifesto ativo, nenhuma página pública é exibida. Vitrine, login, acompanhamento e demais acessos públicos mostram somente o manifesto.</p>
            <div className="mt-4 rounded-xl border border-emerald-300/20 bg-emerald-300/[0.06] p-3 text-xs leading-5 text-emerald-100">O painel administrativo continua acessível para desligar o manifesto ou alterar o temporizador.</div>
          </aside>
        </div>

        <footer className="flex flex-col gap-3 border-t border-white/10 pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-slate-400">Estado atual: <strong className={draft.enabled ? "text-emerald-200" : "text-slate-300"}>{draft.enabled ? "bloqueio total da área pública" : "manifesto desativado"}</strong></p><button type="button" onClick={save} disabled={saveMutation.isPending} className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-cyan-300 to-sky-300 px-5 py-3 text-sm font-black text-slate-950 shadow-[0_0_24px_rgba(34,211,238,.25)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"><Save className="h-4 w-4" />{saveMutation.isPending ? "Salvando manifesto..." : "Salvar manifesto"}</button></footer>
      </div>
    </section>
  );
});
