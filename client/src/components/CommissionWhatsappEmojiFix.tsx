...
function captureWhatsappUrl(
  ...
  try {
    ...
    const encodedHasKnownEmojiUtf8 = [
      ...
      "%F0%9F%8E%89", // festa
      "%F0%9F%91%A4", // pessoa
      "%F0%9F%93%B1", // telefone
      "%F0%9F%92%B0", // dinheiro
      "%F0%9F%92%B3", // cartão
    ...
    <div className="fixed bottom-4 left-4 z-[9999] w-[min(680px,calc(100vw-2rem))] rounded-2xl border border-amber-500/40 bg-[#160f08]/95 p-4 text-white shadow-2xl backdrop-blur">
        ...
        <div className="flex gap-2">
          {record && (
            <button
              ...
              className="rounded-lg border border-amber-400/30 px-2 py-1 text-[10px] font-bold text-amber-100 hover:bg-amber-400/10"
            >
          ...
          <button
            ...
            className="rounded-lg border border-white/15 px-2 py-1 text-[10px] text-white/60 hover:bg-white/10"
          >
            LIMPAR
          </button>
        </div>
      ...
      {!record ? (
        <p className="mt-3 rounded-xl border border-white/10 bg-black/20 p-3 text-xs text-white/70">
          Clique normalmente em <b>WhatsApp</b>, <b>Pedir PIX</b> ou <b>Pagar → Confirmar</b>. O sistema vai registrar exatamente o payload e a URL no instante anterior à abertura do WhatsApp.
        </p>
      ) : (
        <div className="mt-3 space-y-2">
          <div className={`rounded-xl border p-3 ${diagnosis?.level === "problem" ? "border-red-500/40 bg-red-500/10" : diagnosis?.level === "ok" ? "border-emerald-500/40 bg-emerald-500/10" : "border-amber-500/40 bg-amber-500/10"}`}>
            <div className="flex flex-wrap items-center gap-2 text-[11px]">
              <span className="rounded-full bg-white/10 px-2 py-0.5 font-bold">{record.action}</span>
              <span className="text-white/50">origem: {record.source}</span>
              <span className="text-white/50">{new Date(record.capturedAt).toLocaleTimeString("pt-BR")}</span>
            </div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 text-[10px]">
              <div className="rounded-lg bg-black/20 p-2">hasReplacementCharacter: <b>{String(record.payload.hasReplacementCharacter)}</b></div>
              <div className="rounded-lg bg-black/20 p-2">URL contém %EF%BF%BD: <b>{String(record.encodedHasReplacementUtf8)}</b></div>
            </div>
          </div>

          {expanded && (
            <div className="max-h-[48vh] space-y-2 overflow-auto rounded-xl border border-white/10 bg-black/30 p-3 text-[10px]">
              <p className="font-bold text-amber-100">1. TEXTO DECODIFICADO IMEDIATAMENTE ANTES DO WA.ME</p>
              <pre className="whitespace-pre-wrap break-all text-white/80">{record.decodedText}</pre>

              <p className="pt-2 font-bold text-amber-100">2. CODE POINTS / BYTES UTF-8</p>
              <pre className="whitespace-pre-wrap break-all text-white/70">{JSON.stringify(record.payload, null, 2)}</pre>

              <p className="pt-2 font-bold text-amber-100">3. TEXT= PERCENT-ENCODED</p>
              <pre className="whitespace-pre-wrap break-all text-white/70">{record.encodedText}</pre>

              <p className="pt-2 font-bold text-amber-100">4. HREF ORIGINAL DO LINK</p>
              <pre className="whitespace-pre-wrap break-all text-white/60">{record.rawAttribute ?? "window.open: não existe atributo href"}</pre>

              <p className="pt-2 font-bold text-amber-100">5. URL RESOLVIDA</p>
              <pre className="whitespace-pre-wrap break-all text-white/60">{record.resolvedUrl}</pre>
            </div>
          )}
        </div>
      )}
