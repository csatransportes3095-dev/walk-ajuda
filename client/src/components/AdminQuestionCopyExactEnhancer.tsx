...
function validateSourceTree(questions: Question[]) {
  ...
  for (const question of questions) {
    ...
    while (current?.parentQuestionId) {
      if (visited.has(current.id)) throw new Error("A árvore da origem possui uma condição circular.");
...
function buildIdMap(source: Question[], destination: Question[]): Map<number, number> {
  ...
  if (sourceBuckets.size !== destinationBuckets.size) {
    throw new Error("Cópia incompleta: os campos do destino não correspondem à origem.");
  }
...
function sourceChoiceButtons(modal: HTMLElement): HTMLButtonElement[] {
  return Array.from(modal.querySelectorAll<HTMLButtonElement>("button")).filter((button) => {
    const label = normalize(button.textContent || "");
    return label !== "CANCELAR" && !label.includes("COPIAR PERGUNTAS");
  });
}
...
function resolveSourceButton(button: HTMLButtonElement, options: OptionContext[]): OptionContext | null {
  ...
  const countMatch = text.match(/\((\d+) PERGUNTAS?\)/);
  if (countMatch) {
    const count = Number(countMatch[1]);
    const byCount = matches.filter((option) => option.questions.length === count);
...
function resolveDestination(modal: HTMLElement, options: OptionContext[]): OptionContext | null {
  // O modal original renderiza TODAS as opções, exceto a própria opção de destino.
  // Portanto o destino é determinado por ID como a única opção ausente da lista.
  const listedIds = new Set<number>();
  for (const button of sourceChoiceButtons(modal)) {
    const source = resolveSourceButton(button, options);
    if (source) listedIds.add(source.optionId);
  }

  const missing = options.filter((option) => !listedIds.has(option.optionId));
  if (missing.length === 1) return missing[0];

  // Fallback estrito: o próprio modal declara a opção destino dentro do <strong>.
  const declaredLabel = normalize(modal.querySelector("p strong")?.textContent || "");
  if (declaredLabel) {
    const missingWithLabel = missing.filter((option) => normalize(option.optionLabel) === declaredLabel);
    if (missingWithLabel.length === 1) return missingWithLabel[0];

    const allWithLabel = options.filter((option) => normalize(option.optionLabel) === declaredLabel);
    if (allWithLabel.length === 1) return allWithLabel[0];
  }
...
export default function AdminQuestionCopyExactEnhancer() {
  ...
  useEffect(() => {
    ...
    const executeCompleteCopy = async (modal: HTMLElement, confirmButton: HTMLButtonElement) => {
      ...
      if (!destination) {
        toast.error("Não foi possível identificar o destino com segurança. Feche e abra novamente o modal.");
        return;
      }
      if (source.optionId === destination.optionId) {
        toast.error("A opção selecionada é a própria opção de destino. Escolha outra origem.");
        return;
      }

      const confirmed = window.confirm(
        `SUBSTITUIR 100% das perguntas de "${destination.productName} / ${destination.optionLabel}" pelas de "${source.productName} / ${source.optionLabel}"?\n\n` +
      ...
      try {
        ...
        // Reconstrói TODO parentQuestionId com o mapa real origem -> destino.
        // Isso corrige também sub-da-sub independentemente da ordem em que o endpoint criou os registros.
        ...
        toast.success(`Cópia 100% concluída: ${sourceQuestions.length} perguntas e ${manifestCopy.copied} manifesto(s) copiados.`);
        ...
        console.error("[question-copy-exact]", error);
