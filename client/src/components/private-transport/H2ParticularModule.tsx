...
export function H2ParticularModule({ token }: { token: string }) {
  ...
  return (
    <section className="space-y-4 sm:space-y-6">
      <div className="relative isolate overflow-hidden rounded-[22px] border border-cyan-300/25 bg-[linear-gradient(145deg,rgba(8,80,108,.72),rgba(5,14,30,.92)_50%,rgba(17,33,78,.82))] p-4 shadow-[0_18px_46px_rgba(2,12,28,.38),inset_0_1px_0_rgba(255,255,255,.12)] backdrop-blur-xl sm:p-6">
        <div aria-hidden="true" className="absolute -right-10 -top-20 h-56 w-56 rounded-full bg-cyan-300/20 blur-3xl" />
        <div aria-hidden="true" className="absolute bottom-0 left-1/3 h-24 w-2/3 bg-gradient-to-r from-transparent via-cyan-400/10 to-transparent blur-2xl" />
        <span aria-hidden="true" className="absolute inset-x-8 top-0 h-px bg-gradient-to-r from-transparent via-cyan-100/60 to-transparent" />
        <div className="relative flex items-start gap-3 sm:gap-4">
          <div className="flex h-14 w-14 flex-none items-center justify-center rounded-2xl border border-cyan-200/35 bg-cyan-300/10 text-cyan-100 shadow-[0_0_24px_rgba(34,211,238,.18),inset_0_1px_0_rgba(255,255,255,.16)]"><CarFront className="h-7 w-7" /></div>
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-200">H2 Particular</p>
            <h2 className="mt-1 text-2xl font-extrabold tracking-tight text-white sm:text-3xl">Passageiros particulares</h2>
            <p className="mt-1.5 max-w-xl text-sm leading-relaxed text-slate-300">Clientes, agenda, orçamentos, viagens e recebimentos organizados no mesmo lugar.</p>
            <Button onClick={() => setView("appointments")} className="mt-4 h-10 w-full border border-cyan-100/30 bg-gradient-to-r from-cyan-300 via-cyan-400 to-sky-400 font-extrabold text-slate-950 shadow-[0_8px_22px_rgba(34,211,238,.25)] transition-all hover:-translate-y-0.5 hover:brightness-110 sm:w-auto"><CalendarPlus className="mr-2 h-4 w-4" />Novo agendamento</Button>
          </div>
          <div className="hidden rounded-xl border border-emerald-300/20 bg-emerald-400/[.07] px-3 py-2 text-right shadow-[inset_0_1px_0_rgba(255,255,255,.09)] sm:block"><p className="text-[10px] font-bold uppercase tracking-wider text-emerald-200">Motorista</p><p className="max-w-[145px] truncate text-sm font-bold text-white">{bootstrapQuery.data?.user.name || "Carregando..."}</p></div>
        </div>
      </div>
      ...
      <div className="no-scrollbar -mx-1 flex gap-2 overflow-x-auto px-1 pb-1">
        {navItems.map((item) => {
          ...
          return <button key={item.key} onClick={() => { if (item.key === "new-client") { setEditing(false); setForm(emptyClientForm); } setView(item.key); }} className={`group relative flex h-11 shrink-0 items-center gap-2 overflow-hidden rounded-xl border px-3 text-xs font-bold transition-all active:scale-95 ${active ? "border-cyan-300/55 bg-cyan-400/[.16] text-cyan-50 shadow-[0_8px_18px_rgba(34,211,238,0.14),inset_0_1px_0_rgba(255,255,255,.15)]" : "border-white/10 bg-slate-950/55 text-slate-300 shadow-[inset_0_1px_0_rgba(255,255,255,.06)] hover:border-cyan-200/20 hover:bg-slate-900/80"}`}><span aria-hidden="true" className="absolute inset-x-3 top-0 h-px bg-gradient-to-r from-transparent via-white/35 to-transparent" /><span className={`relative flex h-6 w-6 items-center justify-center rounded-lg border ${active ? "border-cyan-200/35 bg-cyan-300/15" : "border-white/10 bg-white/[.035]"}`}><Icon className="h-3.5 w-3.5" /></span><span className="relative">{item.label}</span></button>;
        })}
      </div>
      ...
      {view === "overview" && <div className="space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-[1.35fr_1fr]">
          <div className="relative overflow-hidden rounded-2xl border border-cyan-300/16 bg-[linear-gradient(145deg,rgba(16,51,68,.42),rgba(7,13,28,.86))] p-4 shadow-[0_12px_28px_rgba(1,8,20,.26),inset_0_1px_0_rgba(255,255,255,.08)] backdrop-blur-xl sm:p-5">
...
function StatCard({ label, value, icon, tone, compact = false }: { label: string; value: string; icon: React.ReactNode; tone: "cyan" | "amber" | "emerald" | "violet"; compact?: boolean }) {
  const colors = {
    cyan: { border: "border-cyan-300/25", surface: "from-cyan-400/[.16] via-slate-950/55 to-slate-950/75", text: "text-cyan-100", icon: "border-cyan-200/30 bg-cyan-300/10 text-cyan-100" },
    amber: { border: "border-amber-300/25", surface: "from-amber-400/[.14] via-slate-950/55 to-slate-950/75", text: "text-amber-100", icon: "border-amber-200/30 bg-amber-300/10 text-amber-100" },
    emerald: { border: "border-emerald-300/25", surface: "from-emerald-400/[.14] via-slate-950/55 to-slate-950/75", text: "text-emerald-100", icon: "border-emerald-200/30 bg-emerald-300/10 text-emerald-100" },
    violet: { border: "border-violet-300/25", surface: "from-violet-400/[.15] via-slate-950/55 to-slate-950/75", text: "text-violet-100", icon: "border-violet-200/30 bg-violet-300/10 text-violet-100" },
  }[tone];
  return <div className={`relative overflow-hidden rounded-2xl border ${colors.border} bg-gradient-to-br ${colors.surface} p-3 shadow-[0_9px_20px_rgba(1,6,18,.22),inset_0_1px_0_rgba(255,255,255,.09)] backdrop-blur-xl`}><span aria-hidden="true" className="absolute inset-x-4 top-0 h-px bg-gradient-to-r from-transparent via-white/40 to-transparent" /><div className="relative flex items-start justify-between gap-2"><p className="max-w-[75%] text-[10px] font-bold uppercase tracking-[0.1em] text-slate-300">{label}</p><span className={`flex h-7 w-7 items-center justify-center rounded-lg border ${colors.icon}`}>{icon}</span></div><p className={`${compact ? "text-sm" : "text-xl"} relative mt-2 truncate font-extrabold ${colors.text}`}>{value}</p></div>;
}
