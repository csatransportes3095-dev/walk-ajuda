...
function getWhatsappPhone(parsed: URL): string {
  if (parsed.hostname === "wa.me") return parsed.pathname.replace(/\D/g, "");
  return (parsed.searchParams.get("phone") || "").replace(/\D/g, "");
}

function openOrderOnAndroid(parsed: URL, message: string): boolean {
  if (typeof navigator === "undefined" || !/Android/i.test(navigator.userAgent)) return false;
  if (!message.trimStart().startsWith(ORDER_HEADER)) return false;

  const phone = getWhatsappPhone(parsed);
  if (!phone) return false;

  const fallback = encodeURIComponent(`https://wa.me/${phone}`);
  const intentUrl = `intent://send?phone=${phone}&text=${encodeURIComponent(message)}#Intent;scheme=whatsapp;S.browser_fallback_url=${fallback};end`;
      ...
      try {
        if (url) {
          ...
          if (isWhatsapp && text?.trimStart().startsWith(ORDER_HEADER)) {
            const fullMessage = text.includes(FORM_HEADER)
              ? rewriteWhatsappMessage(text, productsRef.current)
              : text;

            if (openOrderOnAndroid(parsed, fullMessage)) return null;

            if (fullMessage !== text) {
              parsed.searchParams.set("text", fullMessage);
              return originalOpen(parsed.toString(), target, features);
            }
          }
        }
      } catch (error) {
        console.warn("[WhatsAppQuestionTree] Não foi possível preparar a mensagem; mantendo formato original.", error);
      }
