...
export default function H2AdsNewInstanceOrderPicker({ value, onChange }: { value: H2AdsPendingOrderLink | null; onChange: (value: H2AdsPendingOrderLink | null) => void }) {
  ...
  return <div className="rounded-2xl border border-violet-400/25 bg-violet-400/[0.05] p-3">
    <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-violet-200">Vincular cliente ao criar</p><p className="mt-1 text-[11px] leading-4 text-slate-500">Busca o cadastro e mostra cada pedido/subpedido disponível para você vincular o item correto.</p></div>{value && <button type="button" onClick={() => onChange(null)} className="rounded-lg border border-white/10 px-2.5 py-1.5 text-[10px] font-black text-slate-300 hover:bg-white/5">Trocar</button>}</div>
