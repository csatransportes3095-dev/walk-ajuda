...
<div key={date} className="rounded-xl overflow-hidden shadow-md" style={{ background: 'linear-gradient(135deg, #0d0d1a 0%, #070710 100%)', border: '1.5px solid rgba(99,102,241,0.25)' }}>
	...
	<div key={key} className="rounded-xl px-3 py-3 transition-all active:scale-[0.98]" style={{ background: 'linear-gradient(135deg, #052e16 0%, #021a0c 100%)', border: '2px solid rgba(34,197,94,0.7)', boxShadow: '0 2px 10px rgba(34,197,94,0.15)' }}>
	                  <div className="flex items-center gap-2.5 mb-2.5">
	                    ...
	                    <Button type="button" onClick={() => handleAddSingleEarning(key as 'uber' | 'ninetynine' | 'indrive' | 'particular' | 'deliveries' | 'tips' | 'otherEarnings')} disabled={!val || createEarningMutation.isPending} className="h-9 px-3 !bg-emerald-600 hover:!bg-emerald-500 !text-white border-2 border-white/90 text-xs font-extrabold shadow-[0_3px_10px_rgba(5,150,105,0.95)] active:scale-95 transition-all disabled:!bg-emerald-600 disabled:!text-white disabled:!opacity-100 disabled:shadow-[0_3px_10px_rgba(5,150,105,0.75)]">{createEarningMutation.isPending ? '...' : 'Adicionar'}</Button>
	                  </div>
	                  <div className="grid grid-cols-2 gap-2">
	                    <Input
	                      type="number"
