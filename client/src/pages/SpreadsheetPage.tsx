...
<Card className="relative overflow-hidden rounded-xl p-3" style={{ background: 'linear-gradient(135deg, #052e16 0%, #021a0c 100%)', border: '2px solid rgba(34,197,94,0.8)', boxShadow: '0 4px 16px rgba(34,197,94,0.3)' }}>
        ...
        <Tabs value={activeModule} onValueChange={setActiveModule} className="w-full">
          <TabsList aria-label="Módulos da Planilha de Gastos" className="!grid !w-full grid-cols-3 md:grid-cols-5 xl:grid-cols-9 !h-auto !items-stretch gap-2.5 sm:gap-3 bg-transparent p-0 mb-5">
            ...
            <DashboardModuleCard value="analisador" label="Analisador" badge="NOVO" selected={activeModule === 'analisador'} theme={MODULE_THEMES.analisador} icon={<svg className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a10 10 0 1 0 10 10"/><path d="M12 6v6l4 2"/><path d="M20 2v4h-4"/></svg>} />
	...
	<div key={key} className="rounded-xl px-3 py-3 transition-all active:scale-[0.98]" style={{ background: 'linear-gradient(135deg, #2d0808 0%, #180404 100%)', border: '2px solid rgba(239,68,68,0.6)', boxShadow: '0 2px 10px rgba(239,68,68,0.15)' }}>
	                  <div className="flex items-center gap-2.5 mb-2.5">
	                    ...
	                    <Button type="button" onClick={() => handleAddSingleExpense(key as 'fuel' | 'carRental' | 'maintenance' | 'oilChange' | 'washing' | 'insurance' | 'internetPhone' | 'food' | 'parking' | 'tolls' | 'financing' | 'fines' | 'accessories' | 'otherExpenses')} disabled={!val || createExpenseMutation.isPending} className="h-9 px-3 bg-gradient-to-br from-red-400 via-red-500 to-red-700 hover:from-red-300 hover:via-red-500 hover:to-red-600 text-white border border-red-100/70 text-xs font-extrabold shadow-[0_3px_10px_rgba(239,68,68,0.75)] active:scale-95 transition-all disabled:opacity-45 disabled:shadow-none">{createExpenseMutation.isPending ? '...' : 'Adicionar'}</Button>
	                  </div>
...
<div key={date} className="rounded-xl overflow-hidden shadow-md" style={{ background: 'linear-gradient(135deg, #0d0d1a 0%, #070710 100%)', border: '1.5px solid rgba(99,102,241,0.25)' }}>
	...
	<div key={key} className="rounded-xl px-3 py-3 transition-all active:scale-[0.98]" style={{ background: 'linear-gradient(135deg, #052e16 0%, #021a0c 100%)', border: '2px solid rgba(34,197,94,0.7)', boxShadow: '0 2px 10px rgba(34,197,94,0.15)' }}>
	                  <div className="flex items-center gap-2.5 mb-2.5">
	                    ...
	                    <Button type="button" onClick={() => handleAddSingleEarning(key as 'uber' | 'ninetynine' | 'indrive' | 'particular' | 'deliveries' | 'tips' | 'otherEarnings')} disabled={!val || createEarningMutation.isPending} className="h-9 px-3 bg-gradient-to-br from-emerald-300 via-emerald-500 to-emerald-700 hover:from-emerald-200 hover:via-emerald-500 hover:to-emerald-600 text-white border border-emerald-100/70 text-xs font-extrabold shadow-[0_3px_10px_rgba(16,185,129,0.75)] active:scale-95 transition-all disabled:opacity-45 disabled:shadow-none">{createEarningMutation.isPending ? '...' : 'Adicionar'}</Button>
	                  </div>
	                  <div className="grid grid-cols-2 gap-2">
	                    <Input
	                      type="number"
