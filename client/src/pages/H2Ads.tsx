...
function InstanceCard({ instance, profile, hasCredential, assignment, browserRun, worker, workers, busy, actionState, onAssignWorker, onPrepareBrowser, onLaunchBrowser, onCloseBrowser, onDelete, onEditInstance, onEditRoute }: { instance: { id: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }; profile?: NetworkProfile; hasCredential: boolean; assignment?: WorkerAssignment; browserRun?: BrowserRun; worker?: BrowserWorker; workers: BrowserWorker[]; busy: boolean; actionState?: string; onAssignWorker: (instanceId: number, workerId: string) => void; onPrepareBrowser: (instanceId: number) => void; onLaunchBrowser: (instanceId: number) => void; onCloseBrowser: (instanceId: number) => void; onDelete: () => void; onEditInstance: () => void; onEditRoute: () => void }) {
  ...
  return <article className="rounded-2xl border border-white/10 bg-[#10131A]/90 p-4">
    <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h5 className="text-base font-black text-white">{instance.name}</h5><span className="rounded-full border border-[#148CFF]/25 bg-[#148CFF]/10 px-2 py-0.5 text-[10px] font-black uppercase text-[#8CC8FF]">{statusName(instance.status)}</span></div><p className="mt-1 text-xs text-slate-500">{instance.notes || "Rota individual desta instância."}</p></div><button type="button" disabled={isArchived} onClick={onEditRoute} className="inline-flex shrink-0 items-center gap-1.5 rounded-xl bg-[#F5B800] px-3 py-2 text-xs font-black text-[#171003] disabled:opacity-40"><KeyRound className="h-3.5 w-3.5" />{hasCredential ? "Trocar proxy" : "Configurar proxy"}</button></div>
    ...
    <div className="mt-3 rounded-xl border border-emerald-400/15 bg-emerald-400/[0.035] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[11px] font-black uppercase tracking-[0.12em] text-emerald-200">Sessão local</p><p className={`mt-1 text-[11px] ${browserRun?.state === "blocked" ? "text-rose-200" : browserRun?.state === "browser_open" ? "text-emerald-100" : "text-slate-400"}`}>{actionState || browserState}{browserRun?.observedIp ? ` · IP ${browserRun.observedIp}` : ""}</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={instanceBusy || !canPrepare || browserRun?.state === "queued" || browserRun?.state === "preparing" || browserRun?.state === "browser_open"} onClick={() => onPrepareBrowser(instance.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-100 disabled:cursor-not-allowed disabled:opacity-45"><ShieldCheck className="h-3.5 w-3.5" />Preparar</button><button type="button" disabled={instanceBusy || !canLaunch} onClick={() => onLaunchBrowser(instance.id)} className="inline-flex items-center gap-1.5 rounded-lg bg-[#F5B800] px-3 py-2 text-xs font-black text-[#171003] disabled:cursor-not-allowed disabled:opacity-45"><Play className="h-3.5 w-3.5" />Abrir browser</button><button type="button" disabled={instanceBusy || !canClose} onClick={() => onCloseBrowser(instance.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-rose-400/30 bg-rose-400/10 px-3 py-2 text-xs font-black text-rose-100 disabled:cursor-not-allowed disabled:opacity-45"><Square className="h-3.5 w-3.5" />Encerrar</button></div></div><p className="mt-2 text-[10px] leading-4 text-slate-500">A abertura depende de perfil e proxy confirmados. O browser abre apenas no Windows atribuído e não executa automação de sites.</p></div>
    <div className="mt-3 flex items-center justify-between gap-3"><p className="min-w-0 truncate text-[11px] text-slate-500">{profile?.lastCheckMessage || "Nenhuma validação executada."}</p><div className="flex shrink-0 items-center gap-3"><button type="button" onClick={onEditInstance} className="text-xs font-bold text-slate-400 hover:text-white">Editar</button><button type="button" disabled={instanceBusy || browserRun?.state === "browser_open"} onClick={onDelete} className="inline-flex items-center gap-1 text-xs font-bold text-rose-300 hover:text-rose-200 disabled:cursor-not-allowed disabled:opacity-35" title={browserRun?.state === "browser_open" ? "Encerre o browser antes de excluir" : "Excluir instância"}><Trash2 className="h-3.5 w-3.5" />Excluir</button></div></div>
  </article>;
}
