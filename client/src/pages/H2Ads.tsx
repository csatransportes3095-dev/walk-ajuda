...
export default function H2Ads() {
  ...
  const saving = createGroup.isPending || updateGroup.isPending || createInstance.isPending || updateInstance.isPending || saveNetworkProfile.isPending || saveProxyCredential.isPending || validateProxy.isPending || createWorkerPairing.isPending || assignWorker.isPending || revokeWorker.isPending;
...
function GroupSection({ group, instances, profileByInstance, credentialByInstance, workerById, assignmentByInstance, workers, busy, onEditGroup, onNewInstance, onEditInstance, onEditRoute, onAssignWorker }: { group: { id: number; name: string; description: string | null; status: "active" | "archived" }; instances: { id: number; groupId: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }[]; profileByInstance: Map<number, NetworkProfile>; credentialByInstance: Map<number, unknown>; workerById: Map<number, BrowserWorker>; assignmentByInstance: Map<number, WorkerAssignment>; workers: BrowserWorker[]; busy: boolean; onEditGroup: () => void; onNewInstance: () => void; onEditInstance: (instance: { id: number; groupId: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }) => void; onEditRoute: (id: number) => void; onAssignWorker: (instanceId: number, workerId: string) => void }) {
  return <section className="mb-4 overflow-hidden rounded-2xl border border-white/10 bg-black/20"><header className="flex flex-col gap-3 border-b border-white/8 p-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><h4 className="text-base font-black text-white">{group.name}</h4><span className={`rounded-full border px-2 py-0.5 text-[10px] font-black uppercase ${group.status === "active" ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-200" : "border-white/10 bg-white/[0.04] text-slate-400"}`}>{statusName(group.status)}</span></div><p className="mt-1 text-xs text-slate-400">{group.description || "Sem descrição administrativa."}</p></div><div className="flex gap-2"><button type="button" onClick={onEditGroup} className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-2 text-xs font-bold text-slate-300"><Pencil className="h-3.5 w-3.5" />Editar</button><button type="button" disabled={group.status !== "active"} onClick={onNewInstance} className="inline-flex items-center gap-1.5 rounded-lg border border-[#148CFF]/25 bg-[#148CFF]/10 px-3 py-2 text-xs font-bold text-[#8CC8FF] disabled:opacity-40"><Plus className="h-3.5 w-3.5" />Instância</button></div></header><div className="grid gap-3 p-3 lg:grid-cols-2">{instances.length === 0 ? <p className="px-1 py-2 text-xs text-slate-500">Nenhuma instância cadastrada neste grupo.</p> : instances.map(instance => <InstanceCard key={instance.id} instance={instance} profile={profileByInstance.get(instance.id)} hasCredential={credentialByInstance.has(instance.id)} assignment={assignmentByInstance.get(instance.id)} worker={assignmentByInstance.get(instance.id) ? workerById.get(assignmentByInstance.get(instance.id)!.workerId) : undefined} workers={workers} busy={busy} onAssignWorker={onAssignWorker} onEditInstance={() => onEditInstance(instance)} onEditRoute={() => onEditRoute(instance.id)} />)}</div></section>;
}

function InstanceCard({ instance, profile, hasCredential, assignment, worker, workers, busy, onAssignWorker, onEditInstance, onEditRoute }: { instance: { id: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }; profile?: NetworkProfile; hasCredential: boolean; assignment?: WorkerAssignment; worker?: BrowserWorker; workers: BrowserWorker[]; busy: boolean; onAssignWorker: (instanceId: number, workerId: string) => void; onEditInstance: () => void; onEditRoute: () => void }) {
