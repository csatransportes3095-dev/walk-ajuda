...
import { Activity, ArrowLeft, CheckCircle2, Copy, Eye, EyeOff, FolderPlus, KeyRound, Layers3, LockKeyhole, MapPin, Monitor, Network, Pencil, Plus, ShieldCheck, Wifi, WifiOff, X } from "lucide-react";
...
export default function H2Ads() {
  ...
  const saving = createGroup.isPending || updateGroup.isPending || createInstance.isPending || updateInstance.isPending || saveNetworkProfile.isPending || saveProxyCredential.isPending || validateProxy.isPending || createWorkerPairing.isPending || assignWorker.isPending || revokeWorker.isPending || prepareBrowser.isPending;
...
function GroupSection({ group, instances, profileByInstance, credentialByInstance, workerById, assignmentByInstance, browserRunByInstance, workers, busy, onEditGroup, onNewInstance, onEditInstance, onEditRoute, onAssignWorker, onPrepareBrowser }: { group: { id: number; name: string; description: string | null; status: "active" | "archived" }; instances: { id: number; groupId: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }[]; profileByInstance: Map<number, NetworkProfile>; credentialByInstance: Map<number, unknown>; workerById: Map<number, BrowserWorker>; assignmentByInstance: Map<number, WorkerAssignment>; browserRunByInstance: Map<number, BrowserRun>; workers: BrowserWorker[]; busy: boolean; onEditGroup: () => void; onNewInstance: () => void; onEditInstance: (instance: { id: number; groupId: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }) => void; onEditRoute: (id: number) => void; onAssignWorker: (instanceId: number, workerId: string) => void; onPrepareBrowser: (instanceId: number) => void }) {
  return <section className="mb-4 overflow-hidden rounded-2xl border border-white/10 bg-black/20"><header className="flex flex-col gap-3 border-b border-white/8 p-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><h4 className="text-base font-black text-white">{group.name}</h4><span className={`rounded-full border px-2 py-0.5 text-[10px] font-black uppercase ${group.status === "active" ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-200" : "border-white/10 bg-white/[0.04] text-slate-400"}`}>{statusName(group.status)}</span></div><p className="mt-1 text-xs text-slate-400">{group.description || "Sem descrição administrativa."}</p></div><div className="flex gap-2"><button type="button" onClick={onEditGroup} className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-2 text-xs font-bold text-slate-300"><Pencil className="h-3.5 w-3.5" />Editar</button><button type="button" disabled={group.status !== "active"} onClick={onNewInstance} className="inline-flex items-center gap-1.5 rounded-lg border border-[#148CFF]/25 bg-[#148CFF]/10 px-3 py-2 text-xs font-bold text-[#8CC8FF] disabled:opacity-40"><Plus className="h-3.5 w-3.5" />Instância</button></div></header><div className="grid gap-3 p-3 lg:grid-cols-2">{instances.length === 0 ? <p className="px-1 py-2 text-xs text-slate-500">Nenhuma instância cadastrada neste grupo.</p> : instances.map(instance => <InstanceCard key={instance.id} instance={instance} profile={profileByInstance.get(instance.id)} hasCredential={credentialByInstance.has(instance.id)} assignment={assignmentByInstance.get(instance.id)} browserRun={browserRunByInstance.get(instance.id)} worker={assignmentByInstance.get(instance.id) ? workerById.get(assignmentByInstance.get(instance.id)!.workerId) : undefined} workers={workers} busy={busy} onAssignWorker={onAssignWorker} onPrepareBrowser={onPrepareBrowser} onEditInstance={() => onEditInstance(instance)} onEditRoute={() => onEditRoute(instance.id)} />)}</div></section>;
}

function InstanceCard({ instance, profile, hasCredential, assignment, browserRun, worker, workers, busy, onAssignWorker, onPrepareBrowser, onEditInstance, onEditRoute }: { instance: { id: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }; profile?: NetworkProfile; hasCredential: boolean; assignment?: WorkerAssignment; browserRun?: BrowserRun; worker?: BrowserWorker; workers: BrowserWorker[]; busy: boolean; onAssignWorker: (instanceId: number, workerId: string) => void; onPrepareBrowser: (instanceId: number) => void; onEditInstance: () => void; onEditRoute: () => void }) {
  ...
  const canPrepare = Boolean(assignment && worker?.connectionStatus === "online" && hasCredential && profile?.healthStatus === "healthy" && !isArchived);
  const browserState = browserRun?.state === "proxy_verified" ? "Perfil local pronto · proxy confirmado" : browserRun?.state === "queued" ? "Preparação na fila do Worker" : browserRun?.state === "preparing" ? "Verificando a rota pelo Worker" : browserRun?.state === "blocked" ? "Preparação bloqueada: a rota não foi aprovada" : "Browser ainda não preparado";
  return <article className="rounded-2xl border border-white/10 bg-[#10131A]/90 p-4"><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h5 className="text-base font-black text-white">{instance.name}</h5><span className="rounded-full border border-[#148CFF]/25 bg-[#148CFF]/10 px-2 py-0.5 text-[10px] font-black uppercase text-[#8CC8FF]">{statusName(instance.status)}</span></div><p className="mt-1 text-xs text-slate-500">{instance.notes || "Rota individual desta instância."}</p></div><button type="button" disabled={isArchived} onClick={onEditRoute} className="inline-flex shrink-0 items-center gap-1.5 rounded-xl bg-[#F5B800] px-3 py-2 text-xs font-black text-[#171003] disabled:opacity-40"><KeyRound className="h-3.5 w-3.5" />{hasCredential ? "Editar rota" : "Adicionar rota"}</button></div><div className="mt-4 grid grid-cols-2 gap-3 rounded-xl border border-white/8 bg-black/20 p-3 text-xs"><div><p className="text-slate-500">Rota</p><p className={`mt-1 font-bold ${hasCredential ? "text-emerald-200" : "text-slate-400"}`}>{hasCredential ? "Vinculada" : "Não configurada"}</p></div><div><p className="text-slate-500">Último teste</p><p className={`mt-1 font-bold ${healthClass(profile?.healthStatus ?? "not_checked")}`}>{healthName(profile?.healthStatus ?? "not_checked")}</p></div>{profile?.observedIp && <div className="col-span-2 border-t border-white/8 pt-3"><p className="text-slate-500">Resultado</p><p className="mt-1 font-semibold text-slate-200">IP {profile.observedIp}{observedLocation ? ` · ${observedLocation}` : ""}{profile.latencyMs !== null ? ` · ${profile.latencyMs} ms` : ""}</p></div>}</div><div className="mt-3 rounded-xl border border-[#148CFF]/15 bg-[#148CFF]/[0.04] p-3"><div className="flex items-center justify-between gap-2"><p className="text-[11px] font-black uppercase tracking-[0.12em] text-[#8CC8FF]">Worker de execução</p>{worker && <WorkerPresence worker={worker} />}</div><select disabled={busy || isArchived} value={assignment?.workerId ? String(assignment.workerId) : ""} onChange={event => onAssignWorker(instance.id, event.target.value)} className="mt-2 h-9 w-full rounded-lg border border-white/10 bg-black/20 px-2 text-xs font-bold text-slate-200 disabled:opacity-50"><option value="">{workers.length ? "Selecione um Worker" : "Cadastre um Worker primeiro"}</option>{workers.filter(item => item.status === "active").map(item => <option key={item.id} value={item.id}>{item.name} · {item.connectionStatus === "online" ? "online" : "offline"}</option>)}</select><p className="mt-2 text-[11px] text-slate-500">{assignment ? `Perfil: ${assignment.profileState.replace("_", " ")} · versão ${assignment.profileVersion}` : "Ainda sem Worker atribuído."}</p></div><div className="mt-3 rounded-xl border border-emerald-400/15 bg-emerald-400/[0.035] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[11px] font-black uppercase tracking-[0.12em] text-emerald-200">Preparação local</p><p className={`mt-1 text-[11px] ${browserRun?.state === "blocked" ? "text-rose-200" : "text-slate-400"}`}>{browserState}{browserRun?.observedIp ? ` · IP ${browserRun.observedIp}` : ""}</p></div><button type="button" disabled={busy || !canPrepare || browserRun?.state === "queued" || browserRun?.state === "preparing"} onClick={() => onPrepareBrowser(instance.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-100 disabled:cursor-not-allowed disabled:opacity-45"><ShieldCheck className="h-3.5 w-3.5" />Preparar perfil</button></div><p className="mt-2 text-[10px] leading-4 text-slate-500">O browser permanece fechado. A preparação só prossegue com rota aprovada e Worker online.</p></div><div className="mt-3 flex items-center justify-between gap-3"><p className="min-w-0 truncate text-[11px] text-slate-500">{profile?.lastCheckMessage || "Nenhuma validação executada."}</p><button type="button" onClick={onEditInstance} className="shrink-0 text-xs font-bold text-slate-400 hover:text-white">Editar instância</button></div></article>;
}
