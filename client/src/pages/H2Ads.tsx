...
function GroupSection({ group, instances, profileByInstance, credentialByInstance, workerById, assignmentByInstance, browserRunByInstance, workers, busy, onEditGroup, onNewInstance, onEditInstance, onEditRoute, onAssignWorker, onPrepareBrowser, onLaunchBrowser, onCloseBrowser, onDeleteInstance, instanceAction }: { group: { id: number; name: string; description: string | null; status: "active" | "archived" }; instances: { id: number; groupId: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }[]; profileByInstance: Map<number, NetworkProfile>; credentialByInstance: Map<number, unknown>; workerById: Map<number, BrowserWorker>; assignmentByInstance: Map<number, WorkerAssignment>; browserRunByInstance: Map<number, BrowserRun>; workers: BrowserWorker[]; busy: boolean; onEditGroup: () => void; onNewInstance: () => void; onEditInstance: (instance: { id: number; groupId: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }) => void; onEditRoute: (id: number) => void; onAssignWorker: (instanceId: number, workerId: string) => void; onPrepareBrowser: (instanceId: number) => void; onLaunchBrowser: (instanceId: number) => void; onCloseBrowser: (instanceId: number) => void; onDeleteInstance: (instanceId: number, instanceName: string) => void; instanceAction: Record<number, string> }) {
  return <section className="mb-4 overflow-hidden rounded-2xl border border-white/10 bg-black/20"><header className="flex flex-col gap-3 border-b border-white/8 p-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><h4 className="text-lg font-black text-white xl:text-xl">{group.name}</h4><span className={`rounded-full border px-2 py-0.5 text-[10px] font-black uppercase ${group.status === "active" ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-200" : "border-white/10 bg-white/[0.04] text-slate-400"}`}>{statusName(group.status)}</span></div><p className="mt-1.5 text-sm text-slate-400">{group.description || "Sem descrição administrativa."}</p></div><div className="flex gap-2"><button type="button" onClick={onEditGroup} className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-2 text-xs font-bold text-slate-300"><Pencil className="h-3.5 w-3.5" />Editar</button><button type="button" disabled={group.status !== "active"} onClick={onNewInstance} className="inline-flex items-center gap-1.5 rounded-lg border border-[#148CFF]/25 bg-[#148CFF]/10 px-3 py-2 text-xs font-bold text-[#8CC8FF] disabled:opacity-40"><Plus className="h-3.5 w-3.5" />Instância</button></div></header><div className="grid gap-4 p-3 grid-cols-[repeat(auto-fit,minmax(min(100%,340px),1fr))]">{instances.length === 0 ? <p className="px-1 py-2 text-xs text-slate-500">Nenhuma instância cadastrada neste grupo.</p> : instances.map(instance => <InstanceCard key={instance.id} instance={instance} profile={profileByInstance.get(instance.id)} hasCredential={credentialByInstance.has(instance.id)} assignment={assignmentByInstance.get(instance.id)} browserRun={browserRunByInstance.get(instance.id)} worker={assignmentByInstance.get(instance.id) ? workerById.get(assignmentByInstance.get(instance.id)!.workerId) : undefined} workers={workers} busy={busy} actionState={instanceAction[instance.id]} onAssignWorker={onAssignWorker} onPrepareBrowser={onPrepareBrowser} onLaunchBrowser={onLaunchBrowser} onCloseBrowser={onCloseBrowser} onDelete={() => onDeleteInstance(instance.id, instance.name)} onEditInstance={() => onEditInstance(instance)} onEditRoute={() => onEditRoute(instance.id)} />)}</div></section>;
}
...
function InstanceCard({ instance, profile, hasCredential, assignment, browserRun, worker, workers, busy, actionState, onAssignWorker, onPrepareBrowser, onLaunchBrowser, onCloseBrowser, onDelete, onEditInstance, onEditRoute }: { instance: { id: number; name: string; notes: string | null; status: "draft" | "paused" | "archived" }; profile?: NetworkProfile; hasCredential: boolean; assignment?: WorkerAssignment; browserRun?: BrowserRun; worker?: BrowserWorker; workers: BrowserWorker[]; busy: boolean; actionState?: string; onAssignWorker: (instanceId: number, workerId: string) => void; onPrepareBrowser: (instanceId: number) => void; onLaunchBrowser: (instanceId: number) => void; onCloseBrowser: (instanceId: number) => void; onDelete: () => void; onEditInstance: () => void; onEditRoute: () => void }) {
  ...
  return <article className="rounded-2xl border border-white/10 bg-[#10131A]/90 p-5 xl:p-6">
    <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h5 className="text-xl font-black tracking-tight text-white xl:text-2xl">{instance.name}</h5><span className="rounded-full border border-[#148CFF]/25 bg-[#148CFF]/10 px-2 py-0.5 text-[10px] font-black uppercase text-[#8CC8FF]">{statusName(instance.status)}</span></div><p className="mt-1.5 text-sm leading-5 text-slate-400">{instance.notes || "Rota individual desta instância."}</p></div><button type="button" disabled={isArchived} onClick={onEditRoute} className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-[#F5B800] px-4 py-2.5 text-sm font-black text-[#171003] disabled:opacity-40"><KeyRound className="h-3.5 w-3.5" />{hasCredential ? "Trocar proxy" : "Configurar proxy"}</button></div>
    <div className="mt-5 grid grid-cols-2 gap-4 rounded-xl border border-white/8 bg-black/20 p-4 text-sm"><div><p className="text-sm font-semibold text-slate-400">Rota</p><p className={`mt-1.5 text-base font-black ${hasCredential ? "text-emerald-200" : "text-slate-400"}`}>{hasCredential ? "Vinculada" : "Não configurada"}</p></div><div><p className="text-sm font-semibold text-slate-400">Último teste</p><p className={`mt-1.5 text-base font-black ${healthClass(profile?.healthStatus ?? "not_checked")}`}>{healthName(profile?.healthStatus ?? "not_checked")}</p></div>{profile?.observedIp && <div className="col-span-2 border-t border-white/8 pt-3"><p className="text-sm font-semibold text-slate-400">Resultado</p><p className="mt-1.5 text-base font-bold text-slate-100">IP {profile.observedIp}{observedLocation ? ` · ${observedLocation}` : ""}{profile.latencyMs !== null ? ` · ${profile.latencyMs} ms` : ""}</p></div>}</div>
    <div className="mt-3 rounded-xl border border-[#148CFF]/15 bg-[#148CFF]/[0.04] p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs font-black uppercase tracking-[0.12em] text-[#8CC8FF]">Worker de execução</p>{worker && <WorkerPresence worker={worker} />}</div><select disabled={busy || isArchived} value={assignment?.workerId ? String(assignment.workerId) : ""} onChange={event => onAssignWorker(instance.id, event.target.value)} className="mt-3 h-11 w-full rounded-lg border border-white/10 bg-black/20 px-3 text-sm font-bold text-slate-100 disabled:opacity-50"><option value="">{workers.length ? "Selecione um Worker" : "Cadastre um Worker primeiro"}</option>{workers.filter(item => item.status === "active").map(item => <option key={item.id} value={item.id}>{item.name} · {item.connectionStatus === "online" ? "online" : "offline"}</option>)}</select><p className="mt-2.5 text-xs font-medium text-slate-400">{assignment ? `Perfil: ${assignment.profileState.replace("_", " ")} · versão ${assignment.profileVersion}` : "Ainda sem Worker atribuído."}</p></div>
    <div className="mt-3 rounded-xl border border-emerald-400/15 bg-emerald-400/[0.035] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-emerald-200">Sessão local</p><p className={`mt-1.5 text-sm font-semibold leading-5 ${browserRun?.state === "blocked" ? "text-rose-200" : browserRun?.state === "browser_open" ? "text-emerald-100" : "text-slate-400"}`}>{visibleActionState || browserState}{browserRun?.observedIp ? ` · IP ${browserRun.observedIp}` : ""}</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={instanceBusy || !canPrepare || browserRun?.state === "queued" || browserRun?.state === "preparing" || browserRun?.state === "browser_open"} onClick={() => onPrepareBrowser(instance.id)} className="inline-flex items-center gap-2 rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-4 py-2.5 text-sm font-black text-emerald-100 disabled:cursor-not-allowed disabled:opacity-45"><ShieldCheck className="h-3.5 w-3.5" />Preparar</button><button type="button" disabled={instanceBusy || !canLaunch} onClick={() => onLaunchBrowser(instance.id)} className="inline-flex items-center gap-2 rounded-lg bg-[#F5B800] px-4 py-2.5 text-sm font-black text-[#171003] disabled:cursor-not-allowed disabled:opacity-45"><Play className="h-3.5 w-3.5" />Abrir browser</button><button type="button" disabled={instanceBusy || !canClose} onClick={() => onCloseBrowser(instance.id)} className="inline-flex items-center gap-2 rounded-lg border border-rose-400/30 bg-rose-400/10 px-4 py-2.5 text-sm font-black text-rose-100 disabled:cursor-not-allowed disabled:opacity-45"><Square className="h-3.5 w-3.5" />Encerrar</button></div></div><p className="mt-3 text-xs leading-5 text-slate-400">A abertura depende de perfil e proxy confirmados. O browser abre apenas no Windows atribuído e não executa automação de sites.</p></div>
    <div className="mt-3 flex items-center justify-between gap-3"><p className="min-w-0 truncate text-xs font-medium text-slate-400">{profile?.lastCheckMessage || "Nenhuma validação executada."}</p><div className="flex shrink-0 items-center gap-3"><button type="button" onClick={onEditInstance} className="text-sm font-bold text-slate-300 hover:text-white">Editar</button><button type="button" disabled={instanceBusy || browserRun?.state === "browser_open"} onClick={onDelete} className="inline-flex items-center gap-1.5 text-sm font-bold text-rose-300 hover:text-rose-200 disabled:cursor-not-allowed disabled:opacity-35" title={browserRun?.state === "browser_open" ? "Encerre o browser antes de excluir" : "Excluir instância"}><Trash2 className="h-3.5 w-3.5" />Excluir</button></div></div>
  </article>;
}
