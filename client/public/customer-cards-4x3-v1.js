(() => {
  ...
  function compactCustomerCards() {
    ...
    // Abre automaticamente os detalhes. Depois preserva o texto que o filtro legado
    // usa para reconhecer cada card, mantendo o botao invisivel para o ADM.
    document.querySelectorAll('button').forEach((button) => {
      const text = (button.textContent || '').trim();
      if (text.includes('Ver dados e controles') && button.dataset.h2AutoOpened !== '1') {
        button.dataset.h2AutoOpened = '1';
        button.click();
      }
    });
    ...
    document.querySelectorAll('button').forEach((button) => {
      const text = (button.textContent || '').trim();
      if (text.includes('Ocultar detalhes') || (button.dataset.h2AutoOpened === '1' && text.includes('Ver dados e controles'))) {
        button.dataset.h2AutoOpened = '1';
        // customer-card-filters.js procura exatamente este texto para localizar os cards.
        button.textContent = 'Ver dados e controles';
