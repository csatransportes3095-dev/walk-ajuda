...
if (source.includes('data-h2ads-filter="photo-analysis"')) {
  console.log('[h2ads-photo-analysis] OK: filtro ja aplicado e marcado.');
  process.exit(0);
}
...
replaceOnce(
  ...
  '  const pendingScheduleCount = useMemo(() => instances.filter(instance => appointmentByInstance.get(instance.id)?.status === "pending").length, [instances, appointmentByInstance]);\n  const linkedOrderByInstance = useMemo(() => {\n    const orderByKey = new Map<string, (NonNullable<typeof scheduleOrdersQuery.data>)[number]>();\n    for (const order of (scheduleOrdersQuery.data ?? [])) orderByKey.set(`${Number(order.id)}:${Number(order.subOrderIndex ?? 0)}`, order);\n    const result = new Map<number, (NonNullable<typeof scheduleOrdersQuery.data>)[number]>();\n    for (const link of (scheduleLinksQuery.data ?? [])) {\n      const order = orderByKey.get(`${Number(link.registrationId)}:${Number(link.subOrderIndex ?? 0)}`);\n      if (order) result.set(Number(link.instanceId), order);\n    }\n    return result;\n  }, [scheduleLinksQuery.data, scheduleOrdersQuery.data]);\n  const photoAnalysisCount = useMemo(() => instances.filter(instance => getOperationalBucket({ latestStatus: linkedOrderByInstance.get(Number(instance.id))?.latestStatus, scheduleStatus: null }) === "em_analise").length, [instances, linkedOrderByInstance]);\n  const scheduleFilteredInstances = useMemo(() => {\n    if (scheduleFilter === "all") return [] as typeof instances;\n    if (scheduleFilter === "photo_analysis") {\n      return instances\n        .filter(instance => getOperationalBucket({ latestStatus: linkedOrderByInstance.get(Number(instance.id))?.latestStatus, scheduleStatus: null }) === "em_analise")\n        .filter(instance => !instanceSearchKey || normalizeH2AdsSearch(instance.name).includes(instanceSearchKey))\n        .sort((a, b) => a.name.localeCompare(b.name, "pt-BR", { sensitivity: "base" }));\n    }\n    return instances\n      .filter(instance => matchesH2AdsScheduleFilter(appointmentByInstance.get(instance.id), scheduleFilter))\n',
...
replaceOnce(
  ...
  '              <button type="button" onClick={() => { setScheduleFilter("pending"); if (orderingGroups) setOrderingGroups(false); }} className={`rounded-lg border px-3 py-2 text-[10px] font-black transition-colors ${scheduleFilter === "pending" ? "border-amber-300/60 bg-amber-400 text-[#1A1000]" : "border-amber-400/20 bg-amber-400/[0.06] text-amber-100 hover:bg-amber-400/[0.12]"}`}>AGUARDANDO AGENDAMENTO ({pendingScheduleCount})</button>\n              <button type="button" data-h2ads-filter="photo-analysis" title="Filtrar instâncias vinculadas a pedidos com status Foto em análise" onClick={() => { setScheduleFilter("photo_analysis"); if (orderingGroups) setOrderingGroups(false); }} className={`rounded-lg border px-3 py-2 text-[10px] font-black transition-colors ${scheduleFilter === "photo_analysis" ? "border-violet-300/60 bg-violet-400 text-[#14061D]" : "border-violet-400/20 bg-violet-400/[0.06] text-violet-100 hover:bg-violet-400/[0.12]"}`}>FOTO EM ANÁLISE ({photoAnalysisCount})</button>\n',
...
const requiredMarkers = [
  'data-h2ads-filter="photo-analysis"',
  'setScheduleFilter("photo_analysis")',
  'photoAnalysisCount',
  'linkedOrderByInstance',
  'getOperationalBucket',
  'scheduleOrdersQuery',
  'FOTO EM ANÁLISE ({photoAnalysisCount})',
];
for (const marker of requiredMarkers) {
  if (!source.includes(marker)) throw new Error(`[h2ads-photo-analysis] validacao final falhou: ${marker}`);
}
if ((source.match(/data-h2ads-filter="photo-analysis"/g) ?? []).length !== 1) {
  throw new Error('[h2ads-photo-analysis] validacao final falhou: botao duplicado');
}

fs.writeFileSync(file, source, 'utf8');
console.log('[h2ads-photo-analysis] OK: filtro Foto em analise integrado ao H2ADS com origem no status dos pedidos.');
