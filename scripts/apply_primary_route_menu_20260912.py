...
header_close = '      </header>'
start = s.index(header_marker)
end = s.index(header_close, start) + len(header_close)
header = s[start:end]

pattern = re.compile(
    r"\n\s*<button\s*\n\s*onClick=\{\(\) => \{ window\.location\.href = '/acompanhar'; \}\}\s*\n.*?aria-label=\"Acompanhar seu pedido\".*?</button>",
    re.S,
)
header, count = pattern.subn('', header, count=1)
if count != 1:
    raise SystemExit(f'compact tracking button: expected 1, removed {count}')
...
header = header[:-len(header_close)] + cards + header_close
p.write_text(s[:start] + header + s[end:], encoding='utf-8')
